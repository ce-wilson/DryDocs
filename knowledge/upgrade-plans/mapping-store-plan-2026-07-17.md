...
> **O14 (2026-07-18): M2's "DB rebuilt post-merge (M0 script in CI/ritual)" is SUPERSEDED** —
> the store compares its build-time source hashes against the committed sources on every
> connect (`is_current`) and rebuilds itself on drift, so no CI/ritual rebuild hook is needed.
