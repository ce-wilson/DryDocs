...
| T5 | ~~**Multi-statement loader cypher runs via `apoc.cypher.runMany`**~~ — RESOLVED at D5 (2026-07-18): the dependency was dropped entirely. Multi-statement scripts split client-side through `drydocs_core/cypher_split.py`, one auto-commit transaction per statement. | Prefer single-statement `UNWIND` templates where feasible (base already fast-paths these via plain `run()`). | Closed |
