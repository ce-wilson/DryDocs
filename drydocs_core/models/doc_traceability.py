...
class DocSectionRow(BaseModel):
    ...
    doc_type: str = Field(
        ...,
        min_length=1,
        description="TDD | Runbook | Review | Explainer | DesignDoc (stem-suffix rule).",
    )
    rev: int | None = Field(
        None,
        ge=1,
        description="Declared front-matter Rev (design_doc._REV_RE rule); None if undeclared.",
    )
    doc_status: str | None = Field(None, description="DESCRIPTIVE | PRESCRIPTIVE, if declared.")
    commit: str | None = Field(
        None, description="Front-matter `commit \\`hash\\`` citation, if declared."
    )
    path: str = Field(..., min_length=1, description="Repo-relative path, posix-style.")

    # --- DocSection fields ---
    anchor: str = Field(
        ..., min_length=1, description="The authored anchor id (stable, never positional)."
    )
    heading: str = Field(
        ..., min_length=1, description="The heading/first line following the anchor comment."
    )
...
class TraceabilityCitation(BaseModel):
    ...
    kind: str = Field(
        ...,
        min_length=1,
        description="unit | verify | gate | graph-test (open enum, kind-rule-v1).",
    )
    ref: str = Field(
        ..., min_length=1, description="The cleaned citation string (backticks stripped)."
    )
...
class TraceabilityRow(BaseModel):
    ...
    requirement_id: str = Field(
        ...,
        min_length=1,
        description="e.g. 'FR-CMI-003' (per-source format, mode: loose today — gate D5).",
    )
    kind: str = Field(
        ...,
        min_length=1,
        description="FR | UC | NFR | other — derived from the id prefix (gate A3).",
    )
    description: str = Field("", description="The Description cell, verbatim.")
    matrix_status: str | None = Field(
        None, description="The Status cell (done | open | ...), if the table carries one."
    )
...
class FeedbackNoteRow(BaseModel):
    ...
    doc_rev: int = Field(
        ..., ge=1, description="The rev the note was taken AGAINST (filename <N> — gate C2)."
    )
    anchor: str = Field(
        ..., min_length=1, description="The note's anchor as written (may be an L11 derived id)."
    )
    base_anchor: str = Field(
        ...,
        min_length=1,
        description="Authored anchor the note attaches to (derived ids degrade to their parent).",
    )
    note: str = Field(..., min_length=1, description="The note text, verbatim.")
    status: Literal["open", "applied", "rejected", "superseded"] = Field(
        "open",
        description="Review lifecycle (gate C3). Optional per-note in the yaml; defaults open.",
    )
