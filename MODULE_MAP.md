...
| load | `drydocs-load` | `LOAD` | `drydocs.loaders`, `drydocs.cli`, `drydocs.cli_schema`, `drydocs.cli_ingest`, `drydocs.cli_verify`, `drydocs.cli_variables`, `drydocs.cli_docs`, `drydocs.cli_plan`, `drydocs.cli_shared`, `drydocs.cli_consumer`, `drydocs.snapshots`, `drydocs.staging`, `drydocs.cmdline_staging`, `drydocs.seal_samples`, `drydocs.pat_projection`, `drydocs.chain_inputs`, `drydocs.code_graph_freshness`, `drydocs.docs_coverage`, `drydocs.source_registration` |
...
| `drydocs_core/glossary.py` | reads the `drydocs.glossary.v1` files (`config/glossary/terms-public.yaml` plus the Internal twin when present) as a READ surface (R19): `load_glossary()`, `senses_for()`, and `GlossarySense.approved` - a sense counts as approved only at `confidence: confirmed`/`corrected`; a `candidate`/`likely` sense may be offered as a clarification choice, never used to resolve a term silently |
...
| `drydocs_core/check_outcome.py` | **a result names what it did not check (ADR 0021, CORE10):** `CheckOutcome` - the ONE three-outcome type (checked-clean / findings / not-checked-with-a-reason) every probe returns; `bool()` on it raises, every state renders itself, and `PROBES` is the declared registry of adopters that `tests/unit/test_check_outcome.py` reads (each name must resolve and be annotated with the type). First adopters: the port preflight's `base_resolves` and the coverage report's `graph_probe` |
...
| `drydocs_core/data_centers.py` | reads `config/taxonomy/data-centers.yaml` — the DATA-CENTER SPELLING registry (LOAD2): the short Control-M server code (`CM_DEF_VTAB.DATA_CENTER`) paired with the long-form name (`CM_HOSTS` / `CM_AVG_RUN`), so one `--data-center` value reaches every table in the domain that table speaks. `resolve()` matches either spelling and refuses a value in neither, naming both domains; the pairing is DECLARED and never computed (the long form carries a default time and a suffix the short code does not contain). Reads the J13 internal twin where it exists and the publishable sample otherwise, and says which (J18) |
| `drydocs_core/edition_registry.py` | reads `config/taxonomy/editions.yaml` — the EDITION registry (CFG2; gate ontology-domain-registry-and-edition-grain §C1-§C4): the declared id segments, each keyed to an Area Product; `code_collisions()` (never a module series, a frozen letter, or DD — the series and the frozen set are handed in, core imports neither `modules.yaml`'s reader nor the allocator) and `unresolved_area_products()` (a pure compare; the graph read is the caller's, J18). Partitions the ID SPACE; `drydocs_core/ontology/domain_registry.py` partitions the vocabulary, and the two never share (§A2) |
| `drydocs_core/fix_tracking.py`, `drydocs_core/models/remediation.py` | the fix-tracking VOCABULARY and the change-set READER (G90; gate `remediation-fix-tracking` SIGNED 2026-08-12): the schema id, the status enum (§B2), the three ruled property names (§B1), the target NODE KEYs, and `parse_change_set()` — which refuses a foreign schema, an unruled target label, a partial node key, a status outside the enum, a duplicated target, and an envelope property smuggled in (§A1's fence). **Why it is core and not the loader's:** the gate ruled a SEPARATION OF DUTIES — `drydocs_remediation` EMITS the `fix-tracking.v1` artifact and stays no-graph-write (§A2), a drydocs-load loader APPLIES it (§C1) — so two components share one vocabulary, and components never import each other. `drydocs_remediation/changes.py` re-exports the two names it published first, keeping its own tested surface unchanged; this is also what §E1's "drift guard" reduces to, since there is no second copy to drift (pinned by identity, not equality, in `tests/unit/test_fix_tracking_loader.py`). Hoisted rather than added to `DECLARED_COMPONENT_IMPORTS` — the repo carries exactly one such exception and names cross-component imports as the defect | pure parse/validate; **no graph write** (the write is `drydocs/loaders/fix_tracking.py`) |
...
| `drydocs_core/registry_view.py` | **the registry read BY CLASS, one generator for three surfaces** (N26, 2026-09-08): `class_view()` (BDAT layer -> business application -> application id -> ontology class; loaders demoted to a count), `layer_category_matrix()` with its `displaced` predicate (a singleton cell whose category has a home of two or more rows under another layer -- what makes the DPL row visibly odd without a typed field), `ontology_class()` (the node classes of RULED map entries; the edge type rides along as `relationships`; the bare `asset_type` default is UNCLASSIFIED, never a class), `replica_state()` (four states -- origin distinct from carrier corroborated by `authority: ADS`; never a boolean), `application_id_state()`, `loader_binding()` (declared vs effective id after the D2 overlay, and every other loader bound to the same dataset -- the third-loader case), and the STAMP: `input_provenance()` hashes `LOAD_MAP_INPUTS` to git blob ids (CRLF-normalized; `git log --find-object=<blob>` resolves an input to its commits) and `tree_stamp()` names HEAD/branch/dirty (J63). Consumed by `scripts/render_load_map.py` (the By-class section of `docs/plan/load-map.html` and `web/src/generated/load-map.json`) and by `drydocs registry <loader>` in `drydocs/cli_schema.py`, which reads the importable objects (`LOADER_REGISTRY`, `COMMAND_LOADERS`, the `SourceRegistry`, the map fragments) through the same functions (J37). NO NEW REGISTRY FIELD: everything here is derived from fields the registry already carries. Guard: `tests/unit/test_registry_view.py`. **Why it is core:** pure derivation over config facts, loader facts injected as plain dicts; imports no component and writes no graph |
| `drydocs_core/ontology/` | namespace / URN vocab + `relationship_vocabulary.yaml`; `concept_scheme.py` (G77) reads the lob-product-team `skos:ConceptScheme` so the Control-M `THEME` token and the docmeta envelope resolve to one set of concept IRIs; `tom_role_vocabulary.py` (G70) reads the DECLARED TOM role vocabulary `config/taxonomy/tom-role-vocabulary.yaml` — the one surface the seal model, the loader and the supplement seed defer to (gate §A8) — pure config read, no graph write; `domain_registry.py` (CFG1) reads the domain registry `config/taxonomy/domains.yaml`, the one surface the vocabulary's `domain:` axis defers to (gate ontology-domain-registry-and-edition-grain §B1-§B3), same shape |
...
| `drydocs_core/entity_extract.py` | the shared entity/ID extractor (MM3) — pure parse over text returning typed matches with spans: `guid`, `folder_name` (decoded through `orchestration.controlm.parse_folder_name`), `issue_key`, `table_name`, `distribution_list`, `application_id` (a 4-to-7-digit run — the measured SEAL width, CORE5; reported bare from 5 up, and at 4 only when `cued` by a `-seal`/`app_id`/landing-prefix cue). Pass order IS precedence, because the classes overlap on the same text. Core by the placement test (no I/O, no graph, no config); consumed by `drydocs_deepdoc` (mind-map novelty, the MM4/MM5 connectors' "IDs in → references out" contract). Placement is the `drydocs_core` package prefix (`CORE_PREFIXES` in `test_module_boundary.py`) — the physical package is the whole of core since ADR 0002-a-1 |
...
| `drydocs_core/source_descriptors.py` | `drydocs-core` — the five-axis source DESCRIPTOR (`config/source-descriptors.yaml`, source-registration PoC 2026-09-09): `acquisition`, `format`, `authority`, `layer`, `access`, every value drawn from a closed axis, derived from the source registry's system + dataset rows and refined by per-dataset overrides; refuses an off-axis value, an unknown axis or an unknown dataset. Pure config + registry resolve, imports nothing outside core | — (pure; consumed by `drydocs/source_registration/`) |
| `drydocs/source_registration/` | `drydocs-load` (synthetic stand-ins + registration, source-registration PoC) — `synthetic.py` (seeded generator, one CSV table per planned file, every row `record_origin = sample`, ids inside the 70001-70099 block and `example.invalid`), `bundle.py` (ONE gzip `drydocs/data/samples/synthetic-sources.json.gz`, byte-stable, extracted into the loaders' SOURCE-mode landing zones — never into the repo tree), `duckdb_load.py` (optional; `duckdb` imported lazily, tests `importorskip`), `datahub_emit.py` (DataHub MCP file-sink JSON: one container per system, one dataset per registered dataset carrying the five axes as properties and tags). CLI: `scripts/build_synthetic_sources.py`; recipes: `config/datahub/` | bundle under `drydocs/data/samples/`, CSVs and a DuckDB file under `DRYDOCS_DATA_ROOT` (**no graph write**) |
...
| `drydocs/port/reconcile_before.py` | `drydocs-port` — the reconcile-port BEFORE-snapshot as ONE call, STAMPED (2026-09-05): writes the four mandatory before-files the J7/J16 guards read (S5 registries merged, ADR 0013 backlog assembled, gate-log.md byte-for-byte), the two optional J51 lists where their modules import, and `BASE.sha` — the consumer commit the tree was at. Refuses a dirty source (a stamp over an uncommitted edit names a commit the snapshot is not). `check_stamp` is what the guards call first: missing stamp, unresolvable, not an ancestor of HEAD, gate-log differs from `git show <sha>:config/gate-log.md`, or not where the apply branch left main — the last is the company's 2026-09-05 phantom 22nd baseline failure, a before-dir from the EARLIER apply that outlived a skipped teardown (J76: check the instrument first). `describe` prints the one line the PORT-REPORT carries. Pure functions take text and facts; only the git-backed ones shell out. CLI: `scripts/reconcile_before.py [--describe] <before-dir>` | `<before-dir>/` + `BASE.sha`; a one-line report description |
| `drydocs/port/gate_log_redactions.py` | `drydocs-port` — the declared publish-boundary redactions of signed gate-log text (RELAY-47, 2026-09-08): reads `config/gate-log-redactions.yaml` (schema, required fields, `reason: publish-boundary` only - anything else is a rider, L25); `check_append_only` is the line-subsequence append-only rule with a declared replacement standing where a pre-merge line stood LISTED as ruled, never silence and never a failure; `stale_redactions` FAILS a declaration whose replacement is nowhere in the live file. The consumer's guard is a fast-forward and reads the same file, which is why this is a module and a canonical-producer registry rather than an exemption in the test. Holds the AFTER text only. Pure over text and a loaded document | — (a result with `violation` and `ruled`) |
| `drydocs/port/dispositions.py` | `drydocs-port` — the ONE reading of `PORT-MANIFEST.yaml`'s disposition rows (PORT6, 2026-09-08): `classify` (first-match rows, then `default_ok`, else `DEFAULT`) and the anchored glob matcher (`**` spans separators, `*` does not). Extracted from the disposition renderer so the completeness check and the renderer cannot bucket one path differently; `tests/unit/test_port_dispositions.py` asserts the renderer's name IS this function. Pure over a loaded document | — (returns a disposition) |
| `drydocs/port/port_completeness.py` | `drydocs-port` — PORT6 the roll-close COMPLETENESS check: every path at the base tag absent from the consumer tree, by disposition (never-port dropped, canonical-company a ruling band), against the cumulative by-path `deferred-paths` block in the port-prompt AT THE TAG plus `--deferrals` files; and for union-append markdown, every `## ` heading the tag has and the tree lacks. PRESENCE only — `--numstat` answers attribution (this roll or not), this answers completeness (ever applied or not), and the skill's close step runs both. Pure functions take path sets, text and document maps; only the `git_*` helpers and `main` shell out. CLI: `scripts/port_completeness_check.py <base-tag> [--prev TAG]` | the by-class table + COMPLETE / NOT COMPLETE, exit 0/1/2 |
| `drydocs/port/port_drops.py` | `drydocs-port` — PORT4 the file-level port DROP check with the accepted-drop seam: every path removed since the pre-port tag (`git diff --diff-filter=D`, cumulative by construction, so a ruled retirement reappeared at every chunk), by disposition through the one classifier, against the `accepted_drops:` block of the SIDE-LOCAL overlay (`PORT-MANIFEST.*.yaml`, the J34 slot — one side's rulings about its own tree, never carried across and never unioned by the reconcile guards). Three states, none silence: an unruled drop is a finding; a ruled drop is LISTED under its own heading with its report, date and reason; a STALE row FAILS (the path exists again, or nothing dropped it). The `UNION_EXCLUSIONS` shape one level down. Pure over path sets and a loaded document; only the `git_*` helpers and `main` shell out. CLI: `scripts/port_drop_check.py <pre-port-tag>` | the three-heading report + PASS / FAIL, exit 0/1/2 |
