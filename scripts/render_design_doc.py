"""render_design_doc.py — render a design doc .md to .html + .print.html (Epic L / L3).
...
+ ``drydocs.design_doc`` only — the doc .md is the single source of truth; the two HTML
surfaces are deterministic renders of it.
...
def main() -> None:
    ...
    for md in args.docs:
        html_path, print_path = write_doc(md, args.out_dir)
        print(f"wrote {html_path}  +  {print_path.name}")
