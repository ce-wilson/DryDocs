...
THE RANGE HAS TWO SHAPES, AND THE SECOND ARGUMENT IS WHAT TELLS THEM APART. Run
producer-side with one argument, `<base-tag>`, the range is `<base-tag>..HEAD` — the
producer's delta since the last roll, which is what a relay author wants to see. Run
CONSUMER-side, `<base-tag>..HEAD` is the wrong range: HEAD is the apply branch, so it
renders what the branch has ALREADY applied, not what remains, and it does so with a
plausible "wrote N paths" line. The consumer's work plan is the diff between its
pre-apply tree and the producer tag — every path where the two trees differ, the
88-path shape of the 2026-09-01 apply — so the consumer names both ends:
`<pre-apply-tag> <base-tag>`. Until 2026-09-07 `main` read only the first argument and
...
def main(argv: list[str]) -> int:
    if len(argv) > 2:
        print("usage: render_port_dispositions.py [<base-ref> [<head-ref>]]", file=sys.stderr)
        return 2
    ...
    print(f"wrote {OUT.relative_to(REPO)} — {len(paths)} paths in {base}..{head}")
    return 0
