...
INK = "#111a26"; MUT = "#48566c"; EDGE = "#94a3b8"; BG = "#f4f6f9"
PANEL = "#ffffff"; KEYBG = "#2e6bc4"; ACCENT = "#bd1f43"
...
def el_svg(e: dict) -> list[str]:
    ...
    out = [f'<rect x="{x}" y="{y}" width="{w}" height="{h}" rx="{rx}" fill="{PANEL}" stroke="{EDGE}" stroke-width="1.4"{dash}/>']
    # key tag
    out.append(f'<rect x="{x}" y="{y - 16}" width="{9 * len(e["key"]) + 10}" height="18" rx="4" fill="{KEYBG}"/>')
    out.append(f'<text x="{x + 5}" y="{y - 3}" font-family="monospace" font-size="12" fill="#ffffff">{esc(e["key"])}</text>')
    # label, wrapped
    ty = y + 20
    for line in wrap(e["label"], max(24, int(w / 7.5)))[:4]:
        out.append(f'<text x="{x + 12}" y="{ty}" font-family="sans-serif" font-size="13" fill="{INK}">{esc(line)}</text>')
        ty += 17
    # list items
    for it in e.get("items", []):
        out.append(f'<circle cx="{x + 22}" cy="{ty - 4}" r="3" fill="{MUT}"/>')
        out.append(f'<text x="{x + 34}" y="{ty}" font-family="sans-serif" font-size="12.5" fill="{MUT}">{esc(it)}</text>')
        ty += 21
    if kind == "meter":
        out.append(f'<rect x="{x + 12}" y="{y + h - 12}" width="{w - 90}" height="6" rx="3" fill="#e3e9f0"/>')
        out.append(f'<rect x="{x + 12}" y="{y + h - 12}" width="{int((w - 90) * 0.75)}" height="6" rx="3" fill="{ACCENT}"/>')
    return out
