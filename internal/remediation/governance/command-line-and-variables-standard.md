...
- **Distinct labels `INVOKES` vs `USES_ARTIFACT`** (not one label with a role property): *avoids the documented `RUNS_ON` overload risk in `relationship_vocabulary.yaml`* — **directly the same overload that the [loader-flow doc](../../docs/controlm-loader-flow.md) flagged (`RUNS_ON` vs `SCHEDULED_ON`).** The standard chose label-splitting to avoid it.
