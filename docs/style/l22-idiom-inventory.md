...
| `UI-WIP/site-plan.md` (website seed) | 0 | Nothing to do. |
...
|---|---|---|---|
| **"review spine"** | `config/review-labels.yaml` (names itself the review spine), `drydocs/review_labels.py` docstrings, `graph-tests/business-application-identity.yaml` comments | Config header comments, module docstrings, graph-test comments, the drydocs-review component's vocabulary — plus the company-side reproduction (drydocs-review back-flow epic) which inherits the term | Keep for now; if renamed, "review ledger" or "review label registry" — do it before the company back-flow reproduces the term further |
