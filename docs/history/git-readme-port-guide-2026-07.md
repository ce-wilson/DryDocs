# git-readme.md — the July 2026 port guide (superseded; kept as a record)

> **Moved here from `git-readme.md` on 2026-09-02 (ADR 0018 D5, DOC3).** Everything below is
> the port guide as it stood in July 2026 — the commit range, the canonical-here / clean-add /
> collision lists, the Control-M C3/C4 phase tables, the PAT ontology and SOSA deltas, the
> bootstrap order. It was SUPERSEDED by `docs/port/port-prompt.md` (the rolling step ledger and
> the disposition-led apply section) and `PORT-MANIFEST.yaml` (the disposition rows), and the
> paths it names are the paths of that month: `drydocs/controlm/`, `drydocs/models/`,
> `drydocs/schema/` and `drydocs/ontology/` all moved to `drydocs_core/` at the 0002-A
> extraction, and `vendor-bmc-*` became `bmc-docs-*`. Nothing here is a claim about the current
> tree. The live WHY guide stays at `git-readme.md`.
