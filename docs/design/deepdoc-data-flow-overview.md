...
| **Control-M client — job Output tabs** | Everything the names hide (see §3): the shell wrapper, the API endpoint and query, the three files the pre-processor writes, what each FileWatcher actually watched, the launcher's own statement of the job **kind** (`PLACEMENT`, `INGESTION`, `TRANSFORM`, `PROVISION`), the pipeline and dataset GUIDs, the provenance id minted at placement and threaded to ingestion, the landing prefix | Ownership, business purpose, SDLC history |
...
| `provenance_chain` | SOURCE (log) | placement response `provenanceGuid` → ingestion `-proId`; a transform that logs `No provenanceId is provided!` breaks the chain and the record says so |
...
| nothing | the response: `provenanceGuid`, `rowCount`, then a poll of two landing targets until `COMPLETED`, with the landing keys |
