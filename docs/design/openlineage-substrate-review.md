...
| 6 | **`BaseSubsetDatasetFacet`'s condition grammar** | `spec/facets/BaseSubsetDatasetFacet.json` | copy the shape | **The one facet with a ruled consumer.** Field-vs-literal expressions, `compare` (`EQUAL`, `GREATER_THAN`, …), `binary` (`AND`/`OR`), plus partition and location variants — a filter predicate as *data*. It is the declared form for the subset qualifier ruled on 2026-08-30 (two extracts from one table, distinguished by predicate). Take the grammar, not the placement: OpenLineage attaches it as an **input/output** facet, so it is run-scoped and keeps ONE dataset identity, which is right for observed lineage and wrong for a governed registration carrying its own `confirmed`, gate, classification and loader binding. |
| 7 | **The other 37 facet JSON schemas** | `spec/facets/*.json` | none to read | Relevant to the *ontology* layer, not to ADR 0017. Two worth knowing: `TagsDatasetFacet.field` gives column-level classification inside a dataset-level facet, and `HierarchyDatasetFacet` is an ordered database→schema→table array — the credential-free display projection the plan's §2 view needs. |
...
7. Re-derive the facet `$id` versions before using anything in scaffold items 6 or 7. They drift between
