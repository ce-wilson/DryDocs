...
3. **Know which pen you hold before you render.** A default-paths run writes committed
   ...
   ONE lane - the `backlog` pen, per CLAUDE.md section 0 - and the burst's own
   `docs/lane-<x>-handoff.md` names who holds it, generated by the
   [`lane-handoff` skill](../../.claude/skills/lane-handoff/SKILL.md). Render to a scratch
   `--out-dir` if the pen is not yours. Every command in this runbook was run that way.
   The handoff file is cited by SKILL rather than by path on purpose: it is generated per
   burst and deletes itself when its queue empties, so a path citation goes stale every
   time the protocol works correctly (it did, 2026-09-10).
