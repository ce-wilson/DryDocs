...
| FR-REM-2 — detect defects in failure-driven batches + standards sweep | detailed-design (Stage B) | `drydocs_remediation/detect.py` (registry R1 detector) | per-defect-class fixture tests (R1 green) | partial (2026-07-10) |
...
| NFR-REM-1 — no graph write, ever | context-frame; qa-tests | whole component (`corroborate.ReadOnlyGraph` = the sole graph path) | structural AST guard + runtime write-Cypher-refused test (both green) | done (2026-07-10) |
