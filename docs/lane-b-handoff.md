...
generated_at: cbdb590a (main)
queue: [Z9, O87, J76, G134, O54, C43]
pens: [code:drydocs-web, code:config, code:taxonomy]
---
...
```text
pen: code:drydocs-web · code:config · code:taxonomy
```
...
|---|---|---|---|---|---|---|
| 1 | **Z9** | A synthetic city reports that its country has a drawable outline when it has none, because the resolver reads country_id and ignores country_alias | bug / p2 | `drydocs-web` | sonnet | clean |
| 2 | **O87** | The persona picker does not say which seat is the SME — mark it from SME_PERSONA_ID so the green chip follows the constant instead of naming Neo | task / p3 | `drydocs-web` | sonnet | clean |
| 3 | **J76** | The measurement instrument fails toward a reassuring answer - locale decoding and truncated pipelines reported clean where the raw result was not, and one of the three admits a guard | task / p2 | `config` | sonnet | clean |
| 4 | **G134** | The document registry records a corpus's trust level but not the rung that produced it, so a GROUNDED row cannot say whether anything higher was ever tried | task / p2 | `config` | sonnet | clean |
| 5 | **O54** | The enforcement matrix cannot show a guarded code-resident surface as enforced — and the canonical load sequence is exactly that surface | task / p3 | `drydocs-web` | sonnet | clean |
| 6 | **C43** | The software registry does not know mfts or axway because internal branding erased the vendor from every documentation surface - record the platform with its vendor the way controlm carries bmc | task / p2 | `taxonomy` | sonnet | clean |
