...
| `config/doc-source-registry.yaml` *(added after reviewing the company registry, same day)* | NO row — resolves to the `config/**` canonical-producer default. Company `fcdo-frameworks` row still GROUNDED / "no connector has run" with its own ratification citation; producer row is VERBATIM. `graph_locator`, `captured_at`, `manifest` are company-owned facts a wholesale take erases | per-entry by id with a field split: producer fields cross, company fields never do, `confirmed` is per-repo (F4 one file over). See Idea-148 for the run↔row join |
