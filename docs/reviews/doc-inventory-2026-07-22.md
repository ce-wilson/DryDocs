...
| docs/port-prompt-archive-steps-1-42.md | 2026-07-21 | 2026-07-31 | living-reference | Frozen port steps 1–42 (applied via PORT-REPORT). **NOT retirable while T1–T10 are open** — re-labelled from `historical` 2026-07-31: port-prompt.md resolves any "step N ≤ 42" reference here, the Done-means for tracker rows T1–T10 live ONLY in its tracker section, and T5–T8 cite its steps 31/16/16/29 by number. Retire only after T1–T10 close, or after lifting the Done-means block into port-prompt.md first |
| docs/port-ais-supplement-company-prompt.md | 2026-07-28 | 2026-07-31 | hand-off | AIS platform supplement: the back-flow to REFUSE + 3 company-local actions. Retirement condition is its own header — "once the company confirms the actions are landed or declined" — and **tracker T17 is still pending**, so it stays. It carries the repo-wide verification table proving the refusal's premise; retiring it early would leave T17 asserting a refusal whose grounds exist only in git history |
| ~~docs/port-step46-company-prompt.md~~ | 2026-07-27 | 2026-07-31 | RETIRED | Step-46 company-side port prompt (3 corrections to a company-drafted plan). Spent per its own lifecycle note — "retire once the port lands and the PORT-REPORT is written": steps 43–48 applied company-side as PORT-REPORT-94132c80. Its durable content survives where it was folded: the `docs/port-*.md` never-port row and the two ordered depgraph-snapshot rows in PORT-MANIFEST.yaml, and the EXPECTED_CONSTRAINTS standing rule in that manifest's `config/**` note. §5(d)'s unguarded doc-loader warning was closed producer-side by L17. Removed 2026-07-31, recoverable from git history. Precedent: the port-T12 row below |
