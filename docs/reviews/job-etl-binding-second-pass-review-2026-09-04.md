...
- **R8 — MM7 is `in_progress`, not a draft, and has no `wip/` branch.** `git branch -r --list "wip/*"` is empty, so its work is unpushed and invisible (the J31 case). §6-2 rewrites its acceptance: MM7 says *"sets PROPERTIES only… and creates no edge"*, and §6-2 has it staging a binding. Cleaner: the runtime-binding writer is a **new** item that consumes MM7's `OutputCoverage`, leaving MM7 shippable as claimed.
- **R8b — the `provenance_guid` "conflict" is settled, and the paper files it under the wrong hop and the wrong gate.** *(SME clarification, 2026-09-04: it is order-id class — unique for that run — and what it links is a dataset across two zones, S3 bucket to Glue table.)* Three corrections follow.
  **It is already ruled.** SME 2026-08-21 (`8adfa688`): *"a correlation token… not provenance in the PROV-O sense… the guid never keys an edge, identifies a flow, or stands as evidence that a lineage path is real."* The rename `provenance_chain` → `placement_handoff` landed then in the design doc (`deepdoc-data-flow-overview.md:132`) and the gate prompt (`data-flow-overview.yaml:271` carries the SME confirmation verbatim). So there is no *conflict* to settle, and the gate that owns it is **`data-flow-overview`, not MM7's**. What lags is one field name inside MM7's acceptance prose, in a sentence that already states the whole guardrail.
  **It is order-id class, which §4 already covers — in the wrong bullet.** Bullet 2 lists *"business date, **order id**, process id, executing host"*; a run-minted correlation token belongs there. Bullet 1 treats it as a homonym problem and files it under Option C as *"`witness_ref`-adjacent evidence on the binding at most."* That is the wrong hop: its subject is a dataset's zone transition, not the job→ETLProcess binding. F8's *"three values for one dataset in nine days"* is not instability — it is three runs, the token working as designed.
  **And the transition has no object to attach to — which is the finding.** Signed identity is the **GUID alone**, `version` and `zone` as properties (G22 §G clause f, `gate-log.md:2005-2010`), so the S3 and Glue placements of one dataset are **one node**: `glue_tables.py:367-369` writes `glue_path_<zone>` / `glue_database_<zone>` / `glue_table_<zone>` side by side, and the guid is itself a path segment of the S3 landing prefix (`<APP_ID>/raw/<flow>/<guid>/`). G136's title states the consequence — *"a dataset identified by GUID alone turns a raw-to-trusted hop into a self-loop."* A run-scoped id qualifying a self-loop has nowhere to live: not an edge property (no edge), not a node property (overwritten every run). Its owner is the drafted **`dpl-dataset-identity-zone` gate (G136)**, and the paper should point at it exactly the way §6-5 points `:ControlMJobRun` at the `CM_HIST_VW` gate — so the deferral is recorded rather than lost.
...
The four-items-and-a-gate list holds, with two additions, one re-scope and one hand-off: **the curation/promotion item (R2)** — the binding's curation state, a per-binding decision export, and `--confirmed`'s real producer — which is the load-bearing one; **the shared verdict classification in core (R5)**, which absorbs the per-token parser item rather than sitting beside it; and the runtime writer re-scoped to a new item over MM7's coverage (R8). The gate prompt gains: the axis map and the `:Uncertain` question (R2), the promotion-invalidation clause or the derived-grade ruling (R1), the qualified-pattern precedent and its `prov_type` (R3), and the `prov_maps_to` row question (R4) — which is a vocabulary-owner decision either way and now has a surface that forces it. The hand-off is `placement_handoff` (R8b): out of `lineage-binding-class` entirely, out of §4's bullet 1, and named as a deferral to `dpl-dataset-identity-zone` / G136.
