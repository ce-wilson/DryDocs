...
| A4 | Orphan candidates | no in- or out-IMPORTS, `extension = '.py'`, `project IN $packages`, project <> 'tests', name <> '__init__.py', tombstone-filtered | 24 candidates (2026-07-27, six-root) → **0 in-package** at seven roots (2026-08-04) → **0** at eight (2026-08-09, `2d104ef`); every `drydocs_docmeta` module has at least one importer, so the eighth root adds nothing here. Raw whole-repo `.py` reads 77, of which 54 vendored. The first-party non-package queue was `agents/` 15, `scripts/` 4, `knowledge/` 3 = **22 candidates**; after the U19 instrument fix (2026-08-09, depgraph `6ee0af6`) it is **4** — `agents/` **0**, `scripts/` 1, `knowledge/` 3. **Roughly four in five of that queue were an artifact of the scanner, not dead code**, which is why the plan said not to work from it. The four survivors are all subprocess-invoked entry points rather than dead files (`filter_ignored.py` and `probe_instrument.py`, which `snapshot.ps1` runs as scripts; `benchmark_p0.py`; `extract_office_text.py`) — so the honest current reading is zero first-party dead modules and four correctly-identified entry points. Separate queue, never in the 24 baseline |
