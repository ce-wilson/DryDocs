...
*(Items 1–4 closed by the **S2 build, 2026-08-02**. 5–7 remain, with 5 amended.)*

1. [x] **DONE (S2).** `drydocs_core/orchestration/` created; `git mv drydocs_core/controlm drydocs_core/orchestration/controlm`. `drydocs_core/__init__.py` now exports `orchestration` rather than `controlm`.
2. [x] **DONE (S2), with one correction to this ADR.** `paths.py` lifted and split — the neutral module keeps the `FileRef` shape and assembly and gains `PathDialect`, the seam a second vendor supplies instead of forking; the Control-M token vocabulary (`?`-run → `{TS16}`/`{Q<n>}`, `{ODATE}`, unresolved-`%%`, FILEWATCH) is `orchestration/controlm/paths.py`, which re-exports the neutral functions bound to its dialect so behaviour is unchanged. `commands.py` → `orchestration/shell.py`. **THE CORRECTION:** this ADR's table called `commands.py` "Mixed" and scheduled its "PRECMD/POSTCMD/CMD_LINE and EMBEDDED_SHELL/UCM field routing" into `controlm/fields.py`. Measured at the build, that routing was **not in the file** — those names appeared only in its module docstring and two comments, and every executable line was already neutral. The routing lives in `controlm/variables.py` (`SHELL_VAR_NAMES` + the `EMBEDDED_SHELL` classification, correctly already vendor-side) and in `drydocs/staging.py` (component-side, load-cadence — ADR 0002-a §6). `fields.py` therefore exists and is deliberately thin: the Control-M field vocabulary, a `parse_field` router for callers holding a field NAME, and the UCM container-override extraction — the one genuinely Control-M-shaped function `commands.py` did carry. Inventing routing code to fill a planned module would have been worse than recording that the plan over-estimated the split.
3. [x] **DONE (S2).** `orchestration/crosswalk.py` reads `config/crosswalks/*.yaml` and raises `NoEquivalentError` — no nearest-match fallback. Also refuses a `status: proposed` crosswalt at runtime (a proposed crosswalk is a gate's input, not a mapping), and rejects an unknown `fidelity` value because a new tier is a gate decision, not a typo. 16 tests in `tests/unit/test_orchestration_crosswalk.py` over both confirmed crosswalks.
