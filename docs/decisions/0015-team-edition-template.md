...
| [fastapi/full-stack-fastapi-template](https://github.com/fastapi/full-stack-fastapi-template) and the 2026-08-27 SaaS-scaffold survey | Nothing adopted — the surveyed evidence for the rejected scaffold-substrate option, kept so it is not re-litigated | **TRIGGER FIRED 2026-08-31.** The chartered shared estate console is the internal ui-workstream's user/login build. Per this register's own discipline a fired trigger opens a backlog item and auto-changes nothing: the item is O71, and its output is the gate `config/gate-prompts/console-auth-boundary.yaml`, which puts the template's auth PRACTICES (OAuth2 password flow, JWT, hashed credentials, a user model, a recovery flow) to the SME one at a time. **The SCAFFOLD rejection above is unchanged and is not re-opened** — 'What TE takes' stays *nothing adopted*. The ruling itself lands in `config/gate-log.md`, not here, so the row and the record cannot drift. The watch stays OPEN: a rejected scaffold with a fired trigger is still worth watching |
