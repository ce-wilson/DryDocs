...
| [0008 — orchestration module boundary](0008-orchestration-module-boundary.md) | PROPOSED. Add `drydocs_core/orchestration/` as a parent and move `controlm/` under it — do **not** rename `controlm/`, ~1,100 of its 1,725 lines are AutoEdit semantics; lift the neutral shell/path parsers to the parent; `config/crosswalks/` gains a runtime consumer that raises on `fidelity: no-equivalent`; `:ControlM*` graph labels unchanged (ADR 0003 rule 4) |
| [0009 — configuration substrate](0009-configuration-substrate.md) | PROPOSED. Git YAML stays the source of truth (the HITL diff, the commit-range port, the classification tests, and deterministic renders all read files); `var/mapping.db` stays the derived read model and widens; monolith YAML splits by domain + JSON Schema; UI writes go to a `draft` table and are promoted **as a YAML diff onto a branch**, never written to the committed file |
| [0010 — internal source terms](0010-internal-source-term-abstraction.md) | PROPOSED. Identity carries its authority: `:BusinessApplication.app_id` + `id_authority: "SEAL"` (+ optional `urn:dd:…`) replaces `seal_id` as the canonical key — a registry rename becomes a data migration, and a dual-registry transition becomes representable at all; API/web emit the neutral pair from day one; PAT needs no action (it never leaked into code) |
