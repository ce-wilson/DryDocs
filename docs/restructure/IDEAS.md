...
- **`Idea-307`** · 2026-09-08 · `[idea]` · **open** · prio? **Med** —
  **A port applies by DEPENDENCY CLOSURE, not by disposition class: the classifier answers per
  ...
  and `drydocs.port.dispositions.classify` answer one question per PATH: fast-forward, clean-add,
  per-entry, diverged. That answer is correct per path and says nothing about whether the path can
  be applied ALONE. Observed on the 2026-09-08 consumer apply (company-reported; transcription
  machine-local): a clean-add module was taken and imported a symbol from a diverged module that
  was still held, so the taken module failed at collection, eight guards that depended on it
  cascaded, and the consumer ended up holding an entire package subtree plus every changed test
  that imported it - 24 paths - to get a runnable suite. None of the 24 was misclassified. The set
  was.
  THE RULE: the unit a consumer applies is the DEPENDENCY CLOSURE of the paths it takes, not the
  disposition class. A clean-add or fast-forward path whose import edges reach a held path is
  itself held until the held path lands; a per-entry or diverged path drags its importers into the
  same carve-out. Disposition decides HOW a path is taken; the closure decides WHEN, and with what.
  WHY IT IS NOT ALREADY THERE: the SME path-carve-out review carries a `pairs_with` column that
  records exactly this by hand, per row, and nothing reads it. The depgraph snapshot
  (`knowledge/depgraph-snapshots/`) already holds the import edges for every Python module in the
  tree, so the closure is computable at classification time rather than discovered at collection
  time.
  SHAPE IF BUILT: the disposition classifier's report gains a closure pass - for each path in the
  mechanical classes, the set of diverged or held paths its import edges reach (from the snapshot,
  or `tests/source_scan.py`'s `imported_modules`), so the apply plan lists SETS with a shared fate
  and the consumer never learns the set from a stack trace. The `pairs_with` column becomes a
  derived field, checked against the hand-kept one. Non-Python paths (YAML read by a module,
  renders regenerated from a source) are the second-order edges and can stay prose until the first
  order is guarded.
  PROVENANCE: surfaced 2026-09-08 from the consumer session applying `port-base-20260908`;
  the lesson is theirs, the transcription is machine-local. Mechanism only, nothing
  venue-specific. Module drydocs-port. Related [[PORT4]], [[PORT6]], [[Idea-233]].

- **`Idea-306`** · 2026-09-08 · `[idea]` · **groomed → CFG6 (2026-09-08)** · prio? **Med** — **A behavior switch in `config/dev-environment.yaml` that only a test reads is a mechanism whose guard and whose behavior disagree about the same fact; every switch there needs a non-test consumer, or a stated reason it has none.** Source: the consumer's 2026-09-08 port close-out, read against producer `main` the same day (RELAY-45; the review is machine-local under internal-local/research/). THE INSTANCE: `depgraph.capability_assert` was added to `config/dev-environment.yaml` so a consumer whose depgraph fork is separately owned could record a missing scanner capability as an owed action there instead of a defect here. `tests/unit/test_probe_instrument.py` read it and skipped; `knowledge/depgraph-snapshots/snapshot.ps1` never read it and refused unconditionally. On one checkout, in one afternoon, the guard said recorded divergence and the ritual said defect - two halves of one mechanism, one switch, one reader. Fixed at `1a5d30b0` for this key; the CLASS is not fixed. THE SHAPE: a switch is a key whose value changes what some script DOES (refuse or warn, run or skip, assert or record). A switch with only a test consumer is not a switch, it is a test fixture wearing a config key's name, and the script it appears to govern still behaves as if the key were absent. The declaration keys in the same file (`container`, `volume`, `plugins_volume`, `home_db_warning`, `zones_declaration`) are NOT this shape - a test asserting them against `compose.yaml` is a consistency guard between two declarations, which is what a test is for. WHY A GUARD AND NOT A RULE: the flag was introduced with the test half only and looked complete, because the test went green; nothing in the tree asks 'who else reads this'. GUARD SHAPE AT THE GROOM: an allow-list of switch keys in `config/dev-environment.yaml` (today: `depgraph.capability_assert`, and `depgraph.expected_commit` / `expected_branch` which `snapshot.ps1` reads for the currency check), each with at least one non-test consumer found by `tests/source_scan.py` `code_only` over `*.py` plus a comment-stripped read of `*.ps1` (J66 - the flag's name appears in the explanatory comment of the very script that failed to read it), or an inline `# no runtime consumer because ...` beside the key. A new switch with neither fails as UNCLASSIFIED, the default-deny shape `test_module_boundary.py` already uses. WHAT THIS IS NOT: not a request to make every yaml key load-bearing, and not J76 again - J76 is about an instrument that fails toward a reassuring answer; this is about an instrument that exists in one place and is enforced in another. Module config. Related [[J76]], [[J78]], [[J66]].
- **`Idea-305`** · 2026-09-08 · `[idea]` · **groomed → CORE8 (2026-09-08)** · prio? **Med** — **The registry's `origin` label sits at DOMAIN grain, and a carrier-of-carrier chain has no slot for the producing application or the intermediate hops.** Source: a field trace of one people-directory dataset (company-side, transcribed 2026-09-08, machine-local under internal-local/research/; SME-REPORTED), reviewed against the gates and the backlog the same day. The trace found FOUR layers where the registry grammar has two: the bytes are produced by an identity application and a separate HR application, computed by an ETL application's schema, stored on a database platform that is itself a registered business application, and only then copied into the replica the registry names as `system`. The dataset row's `origin` value names the SUBJECT domain of the data - right as a provenance label, wrong as an answer to "which application produced this". The v2 grammar (`origin@db.schema.table`) has one `origin`, one `system` and one `acquisition` block; `config/schemas/source-registry.schema.json` has no producing-application slot and no upstream pointer; `via` (api|db) can hold either hop's value but only for the LAST hop. The two populations inside the one table also arrive by different routes from different producers, which a single acquisition block cannot state. THE HAZARD ALREADY OBSERVED ONCE: the signed fid-identity-and-scope gate treats the table as a carrier with a dataset-on-replica registration and NO new system row (the gate-log reshape of E1), but the signed yaml still reads "system row + dataset row" and is not edited under N13 - so a reader of the yaml alone rebuilds the system row, and one edition did (N25 clause (c)'s "fake HR system" case, seen live). NEAREST OWNERS AND WHY EACH MISSES: C38's `reg_derived_from` edge is dataset-PAIR grain and presumes the intermediate dataset is registered, which would need a system row and a connection block for every hop; N23 owns the carrier slot of the URN for ONE carrier; N25 owns where the layer label lives; Idea-210's "replica: origin != system" is true for one hop. This entry is the owner until groomed. RULE AT THE GROOM: whether `origin` stays a domain label and a producing-application pointer is ADDED (a `produced_by:` naming a business-application id, one per population), or the chain is expressed only through the derivation edge with unconnectable intermediate carriers explicitly allowed. CONSUMERS: the drafted source-connection-and-run-identity gate's mandated stale-premise sweep (A1 restricted record per hop, B2 first non-hypothetical row, C1/C2 hop semantics) and the replica-derivation-edge gate's B3. Module drydocs-core (source registry). Related [[C38]], [[N23]], [[N25]], [[Idea-210]], [[K26]].

- **`Idea-304`** · 2026-09-08 · `[idea]` · **groomed → LOAD6 (2026-09-08)** · prio? **Med** — **The Z4 wiring into the server-resolution pass must answer ONE active data center per load-balancer alias, keyed by city and state - and nothing in the tree defines ACTIVE, so this is a gate ruling before it is an edge.** Source: SME direction 2026-09-08 against L1-1 of the slot-3 lineage review (`docs/reviews/modules/lineage-2026-09-08.md`, grooming candidate 1, amended on `review/module-sweep` at `f8c4953a`). A Control-M job is scheduled against a load-balancer alias; the alias fronts a virtual-server cluster on the order of fifty members; the operational question is never which of the fifty, it is which data center is live for that job now, and country does not discriminate because prod and DR share it. A builder who wires the edge alone emits fifty `RESOLVES_TO_SERVER {match_tier:'dns-resolved'}` edges, satisfies the signed `server-location-ontology` shape, and leaves the question unanswered - so the acceptance of whatever item grooms from this carries the use case, not just the edge. Two scoping facts. First, the renderer half exists: `web/src/components/map/resolve.ts` de-duplicates data centers per site (test "merges rows at one city and de-duplicates data centres and origins"), so the missing piece is the COLLAPSE RULE from N resolved members to one data center, and an item scoped as UI work is scoped wrong. Second, ACTIVE is undefined: an nslookup answer records where the alias pointed AT COLLECTION TIME - an observation, not a status - and Z2's discipline is positive-only observation that never infers absence, so a stale bundle reads "not observed since X" and never "active now". The ruling wanted: what observation makes a data center active for a job, how many observations disagreeing across a bundle collapse (latest wins / majority / all shown with dates), and whether the collapsed answer is a property on the existing edge or its own edge. Expect two items: the gate rider on `server-location-ontology` (or a new prompt), then the build in the load/lineage seam that reads the Z4 evidence file. Module drydocs-load (server-resolution pass; `drydocs_core/ontology/relationship_vocabulary/51-local-infrastructure.yaml`), with drydocs-lineage at the seam. Related [[Z2]], [[Z4]], [[server-location-ontology]].

- **`Idea-303`** · 2026-09-08 · `[bug]` · **groomed → PLAN10 (2026-09-08)** · prio? **Med** — **`scripts/render_gates.py` emits NO row for a gate prompt that self-declares (`^# SIGNED OFF` / `^# DEFERRED`) and has no accounting log entry, so a signed gate whose log record never crossed a port is invisible to `test_gates_json` unless the prompt happens to self-declare - nine of ten missing producer entries on the consumer's gate-log (2026-09-08) rendered as ordinary "session not yet run" rows, and the tenth failed the guard only because its prompt carried the banner.** Source: the machine-local back-flow transcription, twenty-second report (never the captures), and RELAY-37 in `docs/port/port-prompt.md`. The skip at `_SELF_DECLARED_RE` (:60) assumes a self-declared prompt always has its log entry, which is true on the tree that signed it and false on any tree the entry has not reached; the `prompt-only` row for a NON-self-declared prompt with no entry then reads "gate page rendered - session not yet run", which is wrong twice for a DRAFTED-and-recorded gate whose record is missing (it was run; it is recorded; the record is what is absent). Mechanism: (a) a self-declared prompt with no accounting entry gets a row whose title says so ("prompt declares SIGNED OFF / DEFERRED - no log entry accounts for it") with `status: open`, never no row, so the surface shows the gap instead of hiding it and `test_every_log_entry_and_prompt_has_a_row` keeps passing for the right reason; (b) a fixture at the incident's shape (J76): a prompt file with the banner and a log without its heading must produce exactly that row, and adding the heading must remove it. Nothing changes for a prompt whose entry is present. The COMPLETENESS side (does the consumer's log carry every producer heading at the base tag) is PORT6 clause (f), not this; this is the render telling the truth when it does not. Module drydocs-plan (`scripts/render_gates.py`, `tests/unit/test_gates_json.py`). Related [[PORT6]], [[J28]], [[RELAY-37]].

- **`Idea-302`** · 2026-09-08 · `[idea]` · **groomed → PORT7 (2026-09-08)** · prio? **Med** — **The `pyproject.toml` per-entry row says "union of dependencies; keep the consumer's version string" and nothing about `[tool.ruff]`, so the lint ignore list the producer ruled on 2026-08-12 (RUF002/RUF003, ambiguous Unicode in prose) never crossed and the company's copy carries E501/N817 only - one lint standard on both sides is the stated intent (J62 makes `.pre-commit-config.yaml` canonical-producer; the ruff pin upgrades in lockstep) and the row should say so.** Source: the company's carve-out H close (RELAY-36 point three; transcribed to the machine-local back-flow file, nineteenth report). The company read it as a third instance of the per-entry delta-vs-union defect; it is not - no union of the ruff block was ever specified, so none was missed - it is the row being under-specified, which is the same family as the source-registry and dev-environment fall-throughs the manifest already records (a file class sitting under a rule that does not cover it). Mechanism: the `PORT-MANIFEST.yaml:190` entry_rule gains one sentence naming the `[tool.ruff]` block (select, ignore, per-file-ignores, the pin) producer-canonical inside the per-entry file, with the dependency table staying the per-entry part; a relay tells the consumer the block now crosses whole. Lands next roll with PORT6/PLAN9, not mid-apply. Module drydocs-port. Related [[PORT6]], [[J62]], [[RELAY-35]].

- **`Idea-301`** · 2026-09-07 · `[idea]` · **groomed → DOC7 (2026-09-08)** · prio? **Low** — **J31 says when to PUSH a `wip/<id>-<machine>` branch and nothing says when to DELETE the local copy, so the build lane accumulates merged branches that no longer exist on origin - twelve on the laptop at the 2026-09-07 Lane B close, none of them on the remote.** Source: Lane B's close report (the seven-item burst that ended 2026-09-07). Lane A's close step already deletes the REMOTE branch after the `--no-ff` merge (lane-handoff rule 3), so the remote is clean; the local tracking branches on the build machine are the residue, and a stale local `wip/` reads as unmerged work to anyone running the J31 pre-release check from that machine (`git branch -r` is clean, `git branch` is not, and the rule names the remote form only by luck). One sentence, in two places that already exist: the lane-handoff close for Lane B - "after the sender confirms the merge, `git fetch --prune` then `git branch -d` each merged `wip/` branch; `-d` refuses an unmerged one, which is the check" - and the J31 paragraph in CLAUDE.md §0, which names the remote form and should say the local one follows the merge. Not an item of its own unless the sentence turns out to need a script. Module docs. Related [[J31]].

- **`Idea-300`** · 2026-09-07 · `[idea]` · **merged → PLAN8 (2026-09-08)** · prio? **Low** — **REM1 recorded the content of its R10 term-ledger row on its own item because PLAN8 is `todo`, so the row is invisible from the Ready strip: PLAN8 wants a one-line `inputs:` entry naming REM1 so whoever pulls it finds the row without knowing to look.** Source: Lane B's close report; the row sits at `docs/restructure/backlog/items/REM1.yaml` (the "offered to PLAN8's term ledger" note, lines 69-72 at the 2026-09-07 merge) and PLAN8 already carries the pointer to R10 in prose. Lane B did not edit PLAN8 - correctly, an open item another lane may pull is not the build lane's surface - and so the handoff is a note on a DONE item, which nobody re-reads. Mechanism: add `docs/restructure/backlog/items/REM1.yaml` to PLAN8's `inputs:` (an item file is a legitimate input; CORE5 lists K30 and MM3 the same way). Do it as part of the next `backlog`-pen render, not as its own commit. Module drydocs-plan. Related [[REM1]], [[PLAN8]].

- **`Idea-299`** · 2026-09-07 · `[idea]` · **groomed → REM2 (2026-09-08)** · prio? **Med** — **The remediation runbook names exactly one CLI verb, `drydocs profile-folder-set <export-dir>`, and no synthetic Control-M XML export exists in the tree for it to read - so the runbook cannot be demonstrated in a clone, which is the J18 preference (sample-reproducible over machine-pinned) failing on the one document written for the support audience.** Source: Lane B's V7 close (`docs/design/drydocs-remediation-runbook.md`, the verb at line 101 and the "one verb" statement at line 124). The other sample-driven verbs (`ingest-controlm`, the demo interlock) read fixtures under `drydocs/data/samples/`; the folder-set profiler has none, so its only demonstrations have been against real exports on one machine, and the runbook's reader has to take the output shape on faith. Mechanism: a synthetic export in the 70001-70099 block (the same block the demo folders use, so the interlock's app-id read stays true), small - two or three folders, a dozen jobs, one instance of each defect the remediation detectors handle - checked in beside the other samples with a header comment naming the rules it exercises; the runbook's command line then points at it, and a unit test runs the verb against it and pins the report's top-level shape (J76: the instrument gets a fixture). Deliberately synthetic: the generator's job is to be readable, not to be representative. Module drydocs-remediation. Related [[V7]], [[REM1]], [[J18]].

- **`Idea-298`** · 2026-09-07 · `[question]` · **open** · prio? **Low** — **The Windows CI leg costs about 1.9 times the Linux leg per run and runs on every push; nothing has decided whether it earns that on every push or belongs on a schedule.** Source: Lane B's close report (the J67 build, which watched the matrix). The Windows leg exists because both producer machines are Windows and the path-handling class of bug (`repo_paths`, the worktree tests, `PYTHONIOENCODING`) is real and has shipped; that is the argument for keeping it. The argument against every-push is the ratio, and that the Windows-only failures have clustered in a few test families, so a nightly leg plus a per-push run of just those families might catch the same bugs at a fraction of the minutes - a claim to check against the run history, not one this idea asserts. Question, not an item: someone reads the last thirty runs' per-leg durations and Windows-only failures (`gh run list` plus `gh run view --json jobs`) and rules keep / schedule / split. Whatever is ruled goes in the workflow file's own comment, so the next person does not re-ask. Module docs (CI items live there - J10, J67). Related [[J67]], [[J10]].

- **`Idea-297`** · 2026-09-07 · `[question]` · **groomed → DOC8 (2026-09-08)** · prio? **Low** — **The 3.14 advisory leg emits 37 warnings where the 3.12 leg emits 7, and nobody has read the extra thirty - an advisory leg that is never read is a leg that only costs.** Source: Lane B's J67 build (the leg went green on its first run; the warnings summary is in that run's log). Thirty new warnings at a new interpreter are usually a handful of deprecations repeated across many tests, and the triage is one read of the `-rw` summary, deduplicated by message. What comes out is either a short list of fixes (each a one-line change that is safe on 3.12 too) or a ruling that one warning class is a dependency's and is filtered by name in `pyproject.toml` `[tool.pytest.ini_options] filterwarnings`, with the reason. The point is to do it while the count is thirty and the leg is new: the 3.12 baseline of 7 has the same unread history, and this is the moment both get read. Module docs (with J67). Related [[J67]].

- **`Idea-296`** · 2026-09-07 · `[idea]` · **groomed → CORE9 (2026-09-08)** · prio? **Low** — **After CORE4 rewrote the skip-guard policy, `test_data_centers.py`, `test_email_extracts.py` and three more test files pass it WITHOUT their guards - so the guards they carry now exist only to satisfy a policy shape that no longer asks for them, and a guard that protects nothing reads as protection.** Source: Lane B's CORE4 close note (the "reported, not fixed" paragraph; the item's clauses did not cover a sweep). CORE4's finding was that a skip guard written to satisfy J8 rather than to protect anything is worse than no guard, because it looks like a decision. The five files are the same finding one step milder: the guard is real, the policy simply stopped needing it. Cleanup, not a bug: for each file, run the policy test with the guard removed, and if it stays green delete the guard and its docstring sentence, so the only skips left in the tree are ones the policy would refuse to let go. Name the five in the close note; the CORE4 note names two. Module drydocs-core (`tests/unit/test_skip_guard_policy.py` is the instrument). Related [[CORE4]], [[J8]].

- **`Idea-295`** · 2026-09-07 · `[bug]` · **groomed → LOAD7 (2026-09-08)** · prio? **Low** — **`tests/unit/test_pat_projection.py:252` skips with "sample fixture absent on this machine (drydocs/data/ is gitignored)" for a file that is tracked (`git ls-files` lists `drydocs/data/samples/pat_product_mapping__sample.csv`), so the skip can never fire on a clone and the assertion behind it is protected by nothing - the same fake-skip class CORE4 removed from the demo interlock.** Source: Lane B's CORE4 close note, which reported it at line 220; on the merged trunk the skip is at line 252 and the finding is the same. The skip's reason is wrong twice: `drydocs/data/` is gitignored but `drydocs/data/samples/` is carved back in (fourteen tracked files), and the path is NAME-terminated on purpose (`PAT_PRODUCT_MAPPING_FILE`, so the constant cannot drift from the fixture), which is exactly why absence should be a FAILURE and not a skip - a renamed sample must break this test, and today it would skip it. Fix as CORE4 did for the interlock: drop the `exists()` branch, read the file, let a missing fixture fail loudly; one docstring line says why. The test's own concern (the committed fixture and the projection share one header) is right and stays. Module drydocs-load (`drydocs/pat_projection.py` is drydocs-load, G82; the test rides with it). Related [[CORE4]], [[G82]].

- **`Idea-294`** · 2026-09-07 · `[idea]` · **groomed → CORE6** · prio? **Med** — **CORE5 widened the publish-boundary value sweep to seven digits and six values in `knowledge/` surfaced that had never been swept at that width; each was allowlisted by file with a reason, which is the right disposition for every one and the wrong instrument for the class - none of the six is id-shaped, and a value-class exclusion would rule them out by SHAPE and keep the allowlist for values that are genuinely ambiguous.** Source: Lane B's CORE5 close (`tests/unit/test_publish_boundary_values.py`, the allowlist rows added 2026-09-07). The six: five vendor doc-set asset numbers with LEADING ZEROS in `knowledge/upgrade-plans/servicenow-cmdb-analysis.md`, and one public forum thread id that is a URL path segment in `knowledge/upgrade-plans/generic-terminology-research.md`. An application id never starts with zero and never sits inside a URL, so both are excludable by rule: a token with a leading zero is not an id, and a token inside a URL (or immediately after a `/`) is not one either; the width fixtures in `tests/unit/test_entity_extract.py` are a third class (a test file that MUST carry id-shaped values) and stay per-file. Why this matters beyond six rows: the allowlist is the instrument that grows silently - every future widening adds rows, each with a true reason, until the allowlist is longer than the finding list and nobody reads a new row critically; a shape rule is read once. Two clauses: (a) the sweep gains the two exclusions with a fixture each (a leading-zero token and a URL slug at seven digits that the sweep must NOT report, and a bare seven-digit token that it MUST); (b) the six rows are removed and the sweep re-run over the whole publishable tree at the full width, since CORE5 was the first pass ever to look at seven digits and the four-digit end stays unswept by design (CORE5 clause c: every year would match) - that end is the same question and gets its own line in the close note, not a widening. Module drydocs-core (the guard's subject is `drydocs_core/entity_extract.py`'s width belief, and the guard row lives beside it). Related [[CORE5]], [[K30]].

- **`Idea-293`** · 2026-09-07 · `[bug]` · **groomed → CORE7** · prio? **Low** — **`test_repo_paths::test_swept_defaults_resolve_inside_a_real_worktree` spends a real `git worktree add` and fails when pytest itself runs FROM a worktree; it has read as a false "turned green" in two consecutive company carve-out reconciliations - it should skip with a named reason when already inside a worktree.** Source: the 2026-09-07 carve-out I transcription (internal-local/company-backflow/company-apply-port-base-20260905-progress-transcription.md, sixteenth and seventeenth reports). The company's reconcile recipe takes its baseline in a clean worktree at HEAD (no stash - the right call), so every by-test-id comparison sees this one test fail in the baseline and pass in the main tree, and "turned green" has to be discarded by hand each time; the second time it was caught only because the first time was remembered. The test is correct about what it checks (swept defaults resolve inside a REAL worktree) and wrong only about where it may run. Mechanism: at the top of the test, `git rev-parse --git-common-dir` != `git rev-parse --git-dir` means the current checkout is itself a linked worktree; `pytest.skip("runs from the main checkout only - nested worktrees are not the shape under test")`. One assertion in the same file that the skip fires in a worktree and does not fire in the main tree keeps it honest (J76: the instrument gets a fixture). Module drydocs-core (`tests/unit/test_repo_paths.py`). Related [[Idea-291]], [[J76]].

- **`Idea-292`** · 2026-09-07 · `[idea]` · **groomed → PLAN9** · prio? **Med** — **Census pins are venue data living inside canonical tests: `test_docs_coverage::test_the_live_coverage_census_is_pinned` hard-codes the producer's (17, 15, 10, 8) and the company re-pins to its own numbers by hand-merge every roll - the pin belongs in a venue-owned file the test reads, or the test row goes per-entry.** Source: the 2026-09-07 carve-out I transcription (internal-local/company-backflow/company-apply-port-base-20260905-progress-transcription.md, sixteenth report, hand-merges). The test is `tests/**` default_ok and correct as a guard (the census must not drift silently); the tuple inside it is a fact about ONE checkout's corpus, so on every roll it collides, the consumer resolves it by hand, and any later producer edit to the same lines collides again. Same family as the constraint-count row in `test_schema.py` and the fragment count the render lists - a number that is right for exactly one venue, checked in as if it were code. Two shapes, pick one: (a) the tuple moves to a venue file (`config/dev-environment.yaml` already declares `edition:` per venue and is per-entry; a `census:` block there is the smallest move) and the test reads it - the guard keeps its teeth, the data stops crossing; (b) the manifest gains a per-entry row for the test file with an entry_rule naming the pin lines as consumer-owned. (a) is cleaner because it fixes the class, not the file; survey first for the other pins of the same shape (`grep -rn "== (" tests/unit/ | grep -i "census\|count\|pinned"`) so one move covers them all. Module drydocs-plan (the guard) and config (the venue file). Related [[Idea-291]], [[PLAN2]].

- **`Idea-291`** · 2026-09-07 · `[idea]` · **groomed → PORT6** · prio? **High** — **A roll close proves the collision classes complete and never the clean-add class: seven paths and four per-entry rows present at `port-base-20260902` surfaced two carve-outs later as "not in this roll" after that roll closed COMPLETE - a completeness check (`ls-tree -r` at the base tag minus the consumer tree minus never-port / canonical-company rows, expected empty) belongs in the reconcile-port skill and beside `scripts/port_rename_check.py`.** Source: the 2026-09-07 carve-out I transcription (internal-local/company-backflow/company-apply-port-base-20260905-progress-transcription.md, sixteenth and seventeenth reports) and RELAY-35. Two mechanisms, both confirmed by the consumer session itself: (1) its per-entry pass merged the RANGE DIFF where the entry_rule says union by id against the tag's file, so a row added in an earlier range and never merged is in neither diff and stays invisible to every later pass; (2) its attribution instrument (`git diff --numstat <base> <next> -- <path>`, empty = not this roll) prints empty for a forgotten path exactly as it does for a deferred one, and only the lineage pair has a named deferral (T24) - so after a roll closes, deferred and missed are indistinguishable. Mechanism, three parts: (a) a `scripts/port_completeness_check.py` (canonical-producer, ships to the consumer) that takes a base tag and reads `PORT-MANIFEST.yaml` through the same classifier `render_port_dispositions.py` uses, lists every path at the tag that is absent in the working tree and whose class is not never-port or canonical-company, and exits non-zero unless the list is empty or every survivor appears in a deferral file; (b) that deferral file - the roll's deferrals BY PATH, a section the acceptance gate of the port-prompt requires before COMPLETE may be written (T24 is the prose form of exactly one entry); (c) the reconcile-port skill's close step names both instruments together, `--numstat` for "this roll or not" and the completeness check for "ever applied or not". Guard producer-side: a fixture repo where a path lands in range 1, is skipped, and range 2 touches nothing near it - the check must name it at range 2's close (J76: reconstruct the incident at its real shape). Module drydocs-port. Related [[Idea-292]], [[Idea-293]], [[T24]].

- **`Idea-290`** · 2026-09-07 · `[idea]` · **open** · prio? **Low** — **A READ-ONLY profiling work order with a redaction contract and an honesty rule is a reusable template; the company-side registry profile of 2026-09-07 is the first instance and it is worth lifting into a producer skill reference, sanitized.** Source: the 2026-09-07 lineage-workstream transcription (internal-local/research/2026-09-07-lineage-workstream-dpl-registry-gates-transcription.md, the work-order section) and its whole-set review (same directory, `-whole-set-review.md`, action C10). The order's shape is venue-independent: (1) READ-ONLY, named queries only, counts and shapes back, never result rows; (2) a redaction contract stated up front - which token classes may appear in the output at all (finding classes, enum keys, percentages) and which never do (identifiers, names, hosts, values); (3) an honesty rule - a number carries the surface and date it was read from, two surfaces are never blended, and "sample rows" are labelled illustration and never quoted as distribution (three rows had been read as a distribution once in the same session and were superseded on the spot); (4) an explicit list of what the order does NOT answer, so the profile is not read as the ruling. The research-probe-discipline skill has the neighbouring shape (probe first, counts only, venue named - the P1/P6 arrangement) but no template for handing a profiling order to a session that holds a live connection. Candidate: a `references/profile-work-order.template.md` in that skill, with the four parts above and the G65 case as the worked example in mechanism terms. Module: research-probe-discipline. Related [[Idea-289]], [[P6]].

- **`Idea-289`** · 2026-09-07 · `[idea]` · **merged → G136 (2026-09-08)** · prio? **Med** — **Two fields spelled `zone` with opposite behaviour: the payload zone that gate dpl-dataset-identity-zone found degenerate and the registry zone that the G65 profile found multi-valued; G136's candidate mechanism (a catch-all zone key) comes from the registry data and is untested - cross-reference the two in the G136 item so neither session reasons from the other's field.** Source: the 2026-09-07 lineage-workstream transcription and whole-set review (internal-local/research/, finding F5). The G136 prompt reads the `zone` inside dataset_flow.json payloads (one value in practice, so it discriminates nothing); the G65 profile reads the `zone` column of the dataset registry (a multi-valued enum that is a lake layer for object-store rows and a database technology otherwise). Same spelling, different objects, different populations - the J32 shape. G136 §A1 proposes that the payload's single value is a registry catch-all key; that is a hypothesis drawn from the registry profile, not a measured join, and nothing on either page says so. Mechanism, small: the G136 item gains an input line naming the G65 prompt's amended B3/C1-a axis, and its acceptance's stale-premise sweep restates §A1 as a hypothesis to be tested by joining the two fields by GUID, never by name. The G65 prompt already carries the reverse pointer (amended 2026-09-07, header and C1-a). Module config (the gate items). Related [[G136]], [[G65]].

- **`Idea-288`** · 2026-09-07 · `[question]` · **open** · prio? **Med** — **A gate-DRAFTING item can close done with the walk unscheduled; MM2, D10 and the folder-identity draft all did, and each needed a second item minted later to own the RUN - should a drafting item's acceptance name its runner before it closes?** Source: the 2026-09-07 lineage-workstream whole-set review (internal-local/research/2026-09-07-lineage-workstream-whole-set-review.md, finding F3). The pattern is now three deep: the 2026-08-27 sweep minted D11/N19/U10/MM11 for gates whose drafting items had closed; CFG5 was minted 2026-09-07 for a gate drafted eight days after that sweep. MM2's acceptance ("prompt drafted, entries registered planned") was MET - this is not an acceptance defect, it is a policy gap: nothing says WHEN the walk happens or WHO holds the pen for it. Two shapes, and the choice is a HITL ruling on backlog policy, not a guard: (a) a drafting item may not close without a `gates:`-carrying runner item existing (the runner is minted in the drafting item's closing commit, USER-GATED START where the SME sits elsewhere); (b) the drafting item IS the runner - it stays `in_progress` at "drafted, awaiting session" and the board shows it under a drafted-awaiting-session state rather than done. (a) keeps items small and matches the 2026-08-27 precedent; (b) keeps one id per gate but holds items open for weeks. Either way the guard in [[Idea-286]] catches what the policy misses. Module drydocs-plan (the backlog schema / board state), the ruling to the HITL flow doc. Related [[Idea-286]], [[CFG5]], [[D11]], [[MM11]].

- **`Idea-287`** · 2026-09-07 · `[idea]` · **groomed → REV1 (2026-09-08)** · prio? **Med** — **Sweep and triage scripts emit verdicts (BROKEN / MOVED / HOLDS) with no `reviewed_commit`, and four of them on 2026-09-07 called two producer gates broken and one non-existent because the tree they read was behind the base - the J63 block should be part of the script's output, not a habit around it.** Source: the 2026-09-07 lineage-workstream transcription and whole-set review (internal-local/research/, finding F2 and the §1 reclassification table). Four ad hoc sweep scripts (a Control-M precedence sweep, a code-graph sweep, a data-flow gate sweep, a source-connection sweep) each printed a verdict table; none printed the commit, branch or port base it ran against, and the verdicts were read as facts about the producer. Read against producer main at 43ba10fa, two "BROKEN" gates were intact and the "does not exist" gate existed - the classic absent-here-is-not-broken failure that `docs/style/review-provenance.md` names. `scripts/review_stamp.py` already prints the block for a human to paste; the gap is that a script that emits verdicts does not call it. Mechanism, two parts: (1) a `review_provenance()` helper importable from `drydocs_core` (or the stamp script exposing one) that returns the J63 triple, and a one-line rule in review-provenance.md that any script printing a verdict prints the triple first; (2) a guard on the shape J37/J66 already use - a script under `scripts/` whose `called_names` include a verdict-emitting print and not the provenance helper is flagged (code, not prose, per `tests/source_scan.py`). What this does NOT fix: a script run in another repo never sees this guard; the relay carries the rule as a sentence. Module drydocs-review (the helper), docs (the style rule). Related [[J63]], [[Idea-281]] (perishable sections - the same disease in eval cases).

- **`Idea-286`** · 2026-09-07 · `[idea]` · **groomed → PLAN11 (2026-09-08)** · prio? **Med** — **Every unsigned gate prompt should be cited by at least one item that is not done - the 2026-08-27 sweep found thirteen unowned gate runs and fixed them by hand; the folder-identity gate repeated it eight days later and was found by accident, so this wants a guard, not a sweep.** Source: the 2026-09-07 lineage-workstream whole-set review (internal-local/research/2026-09-07-lineage-workstream-whole-set-review.md, finding F1) and the CFG5 mint the same evening. The shape: for every `config/gate-prompts/<id>.yaml`, if `config/gate-log.md` holds no SIGNED (or SME-DEFERRED / BLOCKED / SUPERSEDED) record for `<id>`, then at least one item under `docs/restructure/backlog/items/` carries `<id>` in its `gates:` list with `status` not `done`. Reads the importable objects - the backlog store and the gate-log parser the board already uses for the derived gate QUEUE (J37) - never the rendered gate pages. Two known edge cases to rule in the guard's docstring rather than special-case silently: a gate whose runner is USER-GATED START is still owned (it is `todo`, that is the point); a gate recorded as a RECORD entry (the under-reporting found 2026-08-31 in the derived gate QUEUE: six gates awaiting a session had dropped out as recorded) classifies `recorded` and must count as unsigned for this purpose or the guard inherits the same blind spot. Expected first run: zero findings after CFG5, which is the assertion the guard exists to keep true. Module drydocs-plan (beside `tests/unit/test_backlog.py`'s `gates:` guard at the line that validates gate ids). Related [[Idea-288]], [[CFG5]], [[D11]].

- **`Idea-285`** · 2026-09-07 · `[idea]` · **groomed → DOC9 (2026-09-08)** · prio? **Med** — **The committed `docs/design/*.html` renders have no committed-vs-source guard: the board and roadmap have `test_committed_roadmap_page_matches_its_sources`, the design docs have only the session ritual.** Source: the Lane B close report (laptop, 2026-09-07), inbox hand-back. Lane B committed four governed design renders on its wip branches (three on G127, one on V10) because the items edited the `.md` sources and the ritual says the `.html` is a deterministic render of them - and nothing in CI would have said so either way. `tests/unit/test_design_doc.py` pins the renderer (determinism, single surface per L13, anchors, the rev footer read from the DECLARED rev) but never opens a committed `.html`; the stale-render check in CLAUDE.md section 0 (`render_design_doc.py docs/design/*.md` then `git diff --quiet docs/design/*.html`) is a habit, and Idea-111 already records what a habit-only check costs. Mechanism, on the roadmap guard's shape: for every `docs/design/<stem>.md`, render in memory and compare to the committed `<stem>.html`; report the stems that differ. Two things to settle before writing it, both from the roadmap precedent: (1) the tolerance - Y5 exists because a claim is a status-only edit that ships no render, and design docs have no equivalent one-key edit, so the guard is probably strict, but a `.md` that changes ONLY its `rev:` line under the L5 feedback loop is the case to check; (2) governed renders publish VERBATIM (section 6), so the guard must compare the renderer's bytes, never a normalized form, or a restyled copy passes. Module drydocs-docs (the guard beside `test_design_doc.py`). Related [[Idea-111]], [[L13]], [[G127]], [[V10]].

- **`Idea-284`** · 2026-09-07 · `[bug]` · **groomed → CORE5 (2026-09-07)** · prio? **Med** — **`_APP_ID_RE` in the prose entity extractor is `\d{5}` exactly, and the live SEAL-id population is 4 to 7 digits wide with six digits the common case — the extractor cannot see most real application ids.** Source: the company-side profile of the real PAT team report's `Seal IDs` column (SME-supplied captures, 2026-09-07, transcribed to internal-local/company-backflow/k30-team-details-report-header-list-2026-09-07.md §2; the K30 close note cites the same file). Every token in that column matches `^\d{4,7}$`; five- AND six-digit ids are both common, and a handful are four digits. `drydocs_core/entity_extract.py:135` fixes the width at five (`_APP_ID_RE`), and the module docstring calls an application id "a standalone 5-digit token" throughout — so a six-digit id in a design page, a runbook or a Confluence chunk is never emitted as `application_id`, cued or not. The PAT loading path is NOT affected: `PatProductMappingRow` and `pat_product_mapping.cypher` split on the delimiter with no width rule. Two neighbours to look at in the same fix, not to widen blindly: `tests/unit/test_publish_boundary_values.py:125` scans `\b\d{5,6}\b` for real ids in the publishable tree, which misses a 4- or 7-digit real id by construction (widening to four digits would match every year, so that one wants a cue or an allow-list, not a wider regex); and `tests/unit/test_bundled_demo_interlock.py:47` reads a 5-digit app-id segment out of the demo folder names (synthetic 70001-block, fine as is, but it documents the same belief). Also worth pinning while there: `Team ID` in the same report is a dense integer surrogate key that is SEAL-shaped and is the FIRST column — any shape-matching discovery of "the seal column" picks it first, which is why the projection matches headers by exact name and must keep doing so. Module drydocs-core (the extractor), drydocs-load (the guard rows). Related [[K30]], [[MM3]].

- **`Idea-283`** · 2026-09-07 · `[bug]` · **groomed → LOAD5 (2026-09-07)** · prio? **Med** — **The `test_recursive_sql_cyclic_type_disabled` guard asserts a comment phrase, not the absence of the cyclic-type join from executable SQL - reword the comment and it fails, re-enable the join and it passes (J66).** Source: the UI session's review of the two research-probe-discipline eval cases, 2026-09-07, reviewed at `839eab1f` on `feat/cyclic-type-test-case` (the cases live on that branch, not on main), transcribed to internal-local/research/2026-09-07-eval-case-review.md, finding A. The guard `tests/unit/test_controlm_cypher.py::test_recursive_sql_cyclic_type_disabled` reads the raw file and asserts `"intentionally disabled" in text`. That is a comment-presence check, and the eval case cites it as the thing that keeps the join off ("this case is why it must stay disabled") - which it does not: reword the marker and the guard fails with the join still correctly off; restore the `AND J_SUB.JOB_CYCLIC_IN = D_SUB.JOB_CYCLIC_OUT` predicate (`drydocs/loaders/sql/controlm_dependencies_recursive.sql:88`, commented out today) with the phrase surviving anywhere in the file and the guard stays green. The right helper is two functions up in the same file: `_sql_code()` (:29) strips `--` tails per J26, and the sibling test at :451 already uses it. Fix: assert the COMPARISON is absent from `_sql_code(...)` - a pattern on `JOB_CYCLIC_IN`, optional whitespace, `=`, then `JOB_CYCLIC_OUT` - not the bare token, because `JOB_CYCLIC_IN` is a live SELECT alias at :57 and a bare-token assertion would fail on the alias. Keep a comment-presence check only if the marker itself is contract (J66's stated exception, said in the test). Independent of everything else in this batch and the smallest safe thing to land. Module drydocs-load. Related [[Idea-273]] (a positive control belongs beside the new absence assertion).

- **`Idea-282`** · 2026-09-07 · `[idea]` · **groomed → API4 (2026-09-07); clause (c), the chunk-level trust override, is SCOPED on the item and NOT decided - the docmeta trust-axis ruling stays open** · prio? **Med** — **Term search over `Chunk.text` and a chunk-level trust override are the two follow-ups the cyclic-type-trap eval case named and never inboxed; `pipeline.py` hardcodes `chunks: 0`, so no /ask tier reads chunk text today.** Source: the UI session's review of the two research-probe-discipline eval cases, 2026-09-07, reviewed at `839eab1f` on `feat/cyclic-type-test-case` (the cases live on that branch, not on main), transcribed to internal-local/research/2026-09-07-eval-case-review.md, finding B as AMENDED in the same session (its first form was partly wrong and would have sent someone to rebuild `ask.file-search.v1`, which O62 built as the first term-accepting spec - over `DataAsset.assetId`, graph assets). What is genuinely un-inboxed: (1) a term-searching docs QuerySpec over `Chunk.text` or a full-text index - `docs.search.v1` searches Document title/abstract and calls itself Chunk-free (`drydocs_api/query_specs.py:946`); (2) un-hardcoding `chunks: 0` in `agents/graph_qa/pipeline.py` (:362 and :535), which is why no /ask tier reads chunk text; (3) a chunk-level trust override for the illustrative `ctmdefine` enum the cyclic-type-trap case flags - loaded GROUNDED by document default with its hazard banner in a different chunk, the case's own "trust defect". Also gives the smuggling-dot case's B1 evidence a home: `_consume_delimiter` (`drydocs_core/orchestration/controlm/resolver.py:141`) consumes a period only when `.%%` follows, so the `..` spelling over-emits - reproduced by running the three shapes, not read off the table. This is the gate on [[Idea-279]]'s vendor-corpus option. Module drydocs-api (the spec), drydocs-agents (the pipeline); the override belongs to the docmeta trust axis. Related [[O62]], [[Idea-279]].

- **`Idea-281`** · 2026-09-07 · `[idea]` · **groomed → PLAN7 (2026-09-07)** · prio? **Low** — **An eval case that pins a sha should say which of its sections are sha-sensitive: the two research-probe cases are 367 commits behind on exactly the console-arm findings that moved.** Source: the UI session's review of the two research-probe-discipline eval cases, 2026-09-07, reviewed at `839eab1f` on `feat/cyclic-type-test-case` (the cases live on that branch, not on main), transcribed to internal-local/research/2026-09-07-eval-case-review.md, finding C. Both cases carry `checked_against: origin/main dbd13170` - correct J63 form, real commit - and origin/main is 367 commits past it, with the churn on the console arm the cases assess: R19 clarification tier, R15 epistemic labeling, O63 reachability, WEB8, WEB10. Their core conclusions hold at `839eab1f` (`chunks: 0` unchanged; no registered docs spec searches chunk text) but the "the console arm cannot answer this today" sections are dated 2026-09-02/03 and perish with every console commit. Mechanism, small: an eval case that pins a sha marks which sections are sha-sensitive (a `perishable:` list beside `checked_against`, or a per-section marker), so a re-check re-runs those and cites the rest as standing. Precedent is the review-provenance block itself - it names the tree; this names which claims depend on it. Module: the research-probe-discipline skill's eval shape. Related [[J63]].

- **`Idea-280`** · 2026-09-07 · `[idea]` · **groomed → REM1 (2026-09-07)** · prio? **Low** — **`R10` and `R1` name two unrelated things - a remediation rules-registry rule and a backlog item - so a bare grep lands on the wrong artifact; cite registry rules as `registry R10` on first use.** Source: the UI session's review of the two research-probe-discipline eval cases, 2026-09-07, reviewed at `839eab1f` on `feat/cyclic-type-test-case` (the cases live on that branch, not on main), transcribed to internal-local/research/2026-09-07-eval-case-review.md, finding D. The smuggling-dot case cites "Rule R10 in the registry" - concatenation-dot correctness, `internal/remediation/standards-rules-registry.md:57`. `docs/restructure/backlog/items/R10.yaml` is an unrelated closed chore about pinning an agent dependency. The case is precise ("in the registry"), but a grep for `R10` lands on the wrong artifact, and `R1` has the same collision (`DOT_SMUGGLING_RULE_ID = "R1"` in `drydocs_remediation` detect.py:48 against backlog R1). Not a defect in either series - the R letter was frozen with the other 27 on 2026-09-02, so no renumbering. Convention only: cite a rules-registry rule as `registry R10` on first use in any prose, and the standards-rules registry says so in its header. Candidate for [[Idea-278]]'s ledger as a row ("R10" -> two homes). Module drydocs-remediation (the registry header line).

- **`Idea-279`** · 2026-09-07 · `[idea]` · **groomed → AGENT1 (2026-09-07); the vendor-corpus option stays gated on API4 and the general-knowledge option on the R19/R15 check, both carried on the item** · prio? **Med** — **The /ask scope selector is a ROUTER HINT that filters the spec catalog at `pipeline._route`, not a second retrieval path - and its vendor-corpus option must not ship until a spec searches chunk text.** Source: the UI session's review of the two research-probe-discipline eval cases, 2026-09-07, reviewed at `839eab1f` on `feat/cyclic-type-test-case` (the cases live on that branch, not on main), transcribed to internal-local/research/2026-09-07-eval-case-review.md, decision D1 (agreed in session, not built). The dropdown in front of "Ask the knowledge graph" CONSTRAINS the spec catalog the LLM router already sees: `agents/graph_qa/pipeline.py::_route` joins `specs_catalog.catalog_lines()` into the router system prompt; a scope filters that list before the join. No new tier, no second backend; the request envelope carries the scope so the answer reports which one ran. Readiness is NOT uniform and that is the finding to hold on to: knowledge graph = ready (registered specs + text2cypher); vendor corpus = NOT ready, because `docs.search.v1` is title/abstract only and `pipeline.py` hardcodes `chunks: 0` - a control LABELLED vendor corpus would turn O62's illusion (a full listing read as a search hit by title similarity) into a feature, so that option is gated on [[Idea-282]]; general knowledge = needs a distinct R15 epistemic label before it ships, and overlaps R19's clarification tier, which already handles "the user does not know the subject" by asking - check that first. Surfaces: `code:drydocs-agents` (filter + envelope), `code:drydocs-web` (the control). Related [[O62]], [[R15]], [[R19]], [[Idea-282]].

- **`Idea-278`** · 2026-09-07 · `[idea]` · **groomed → PLAN8 (2026-09-07); which of the two empty shells gets filled is a recommendation on the item, confirmed against the schema before writing** · prio? **Med** — **A synonym -> canonical term -> home ledger for Control-M knowledge, filled into the research-terms shell that already exists (three verified seed rows), instead of a fifth skill.** Source: the UI session's review of the two research-probe-discipline eval cases, 2026-09-07, reviewed at `839eab1f` on `feat/cyclic-type-test-case` (the cases live on that branch, not on main), transcribed to internal-local/research/2026-09-07-eval-case-review.md, decision D2 (agreed in session, not built). The failed research session's problem was vocabulary, not search: the smuggling-dot case records that a grep for "period" reaches none of six sources because the prose says "dot", and `docs.data-centers.v1` EXISTS as a registered spec while a session could not find data-center material. The deliverable is an INDEX, not a procedure: synonym -> canonical term -> home, across the four places Control-M knowledge lives (`external/orchestration/bmc-controlm/`, `knowledge/standards/technology/`, `internal/remediation/standards-rules-registry.md`, `.claude/skills/controlm-db/references/er-model.md`). Two empty shells already exist - fill one, do not add a fifth surface: the research ledger (`.claude/skills/research-probe-discipline/references/terms.template.yaml`, schema `drydocs.research-terms.v1`, whose `internal/research/_registry/terms.yaml` instance was never created) or `internal/glossary/terms.yaml` (G34 scaffold, `drydocs.glossary.v1`, business definitions). A synonym-to-location map is the research ledger's shape; confirm before writing, both are tracked. Three seed rows verified at `839eab1f`: period/dot/concatenate/`..` -> dot-smuggling vs the concatenation delimiter (registry rule R1, the greenfield standard :179, resolver.py:21, detector field variables.py:272); cyclic / `C` / `S` -> `CYCLIC` is the Y/N flag and `CYCLIC_TYPE` the cycle type, NOT is-cyclic (the trap itself; the letter map stays To-verify - the ledger records the question, not a guess); daily vs cyclic -> folder suffix `_CYC`/`_DLY`, daily inferred from absence. If a skill wrapper helps discovery, extend `controlm-db` ("which table holds X") to "which document explains X" rather than add one. Converges with [[Idea-277]] - the same missing term index from the console side; do this first so the Tower acronym link has a target. Module: research-probe-discipline (the ledger), controlm-db. Related [[G34]], [[Idea-280]].

- **`Idea-277`** · 2026-09-07 · `[idea]` · **groomed → WEB18 (2026-09-07); the generated-json versus live-spec question stays OPEN on the item - it is the render pen's call** · prio? **Med** — **The Explorer header puts the word Tower beside `backs onto drydocs` - the console half of the R22 provenance fix is missing; a hover sourced from `ui-concepts.yaml`, never hand-written, with one open question (generated json or live spec).** Source: the UI session's review of the two research-probe-discipline eval cases, 2026-09-07, reviewed at `839eab1f` on `feat/cyclic-type-test-case` (the cases live on that branch, not on main), transcribed to internal-local/research/2026-09-07-eval-case-review.md, decision D4 (agreed in session, not built). `web/src/routes/ModuleTemplate.tsx:51` renders every module header as tagline, a dot, then "backs onto" the declared backend; for Explorer (`web/src/modules/registry.ts:60`) that reads "Tower / app drill-down graph - backs onto drydocs". `backsOnto: 'drydocs'` is CORRECT - the tabs read the graph - but the word Tower beside it invites the exact inference R22 exists to prevent: on 2026-08-20 "how many towers" fell through to text2cypher and the model proxied the console term onto `:TOMRole`, crossing from UI taxonomy into the TOM ontology unannounced. R22 fixed /ask (`answer_declared`, `agents/graph_qa/pipeline.py:487`, answers from the declaration with its provenance); the Explorer header never got the fix. Deliverable: a hover and/or link on the header term, SOURCED from `config/taxonomy/ui-concepts.yaml` (schema `drydocs.ui-concepts.v1`; source `web/src/data/towers.ts`, `graph_binding: none`, aliases incl. the CTO spelling; guarded by `tests/unit/test_ui_concepts.py`) and never hand-written - a second definition drifts from the guarded one. Two constraints: it may say "defined in the console, not from the graph, four"; it may NOT assert any ontology binding (`Tower -> :TOMRole` or other) - that is a HITL gate decision, `status: planned` first. OPEN QUESTION, decided before building: there is no `web/src/generated/ui-concepts.json`, so either (a) a render step emits one (the gates/matrix/software-registry precedent; drift-guarded; a header caption should not depend on a live request) or (b) the header calls `docs.ui-concepts.v1` at render time (no new file; a request and a failure mode on a page header). Precedent favors (a); it is a render-surface change and the render pen's call. Cheap win now: surface `aliases` in the hover; a real acronym LINK waits on [[Idea-278]]. Module drydocs-web (+ the render step if (a)). Related [[R22]], [[Idea-278]].

- **`Idea-276`** · 2026-09-07 · `[idea]` · **groomed → CFG4 (2026-09-07)** · prio? **Med** — **A UI dependency is NOT_ALLOWED in the company technology-governance catalog, and the company says the UI will move onto its ADK-based data-marketplace platform - the software-registry needs a governance-status read on its OWN dependencies, not only on the estate it documents.** Source: the company-side session of 2026-09-07 that reconciled all 16 software-registry products against the technology-governance catalog through its MCP server (transcribed to internal-local/research/2026-09-07-c33-governance-catalog-mcp-capture.md, section 2; the images are not the source). The catalog rules one of the web UI's graph-rendering dependencies NOT_ALLOWED while the framework it sits on is ALLOWED, and the company user's note is that the change is known and the UI will flip to the company data-marketplace platform, which is built on the Google ADK. Two things for the producer, neither a rewrite: (1) the registry's company-side governance enrichment (mechanism = per-product governance block with `as_of`, ids never names, absence recorded) has a producer counterpart only as a SCHEMA - the producer has no catalog, so its rows carry `governance: ~` and a guard that the block, when present, has the mandatory `as_of`; (2) the two-track UI plan (UI-WIP/two-track-ui-plan.md) gains a dated note that the company's target UI stack is an ADK-based marketplace and the producer's ReUI/graph-rendering choice is producer-only - which matters for what the port carries under `web/`. Governance status has no authority over the ledger; it annotates. Module config (the schema half), web (the note). Related [[Idea-221]], [[N24]], [[Idea-275]].

- **`Idea-275`** · 2026-09-07 · `[source]` · **merged → C33 (2026-09-07), on its own instruction; the closed-fid-population detector candidate is recorded there and not built** · prio? **Med** — **The other ETL platform in the software-registry is DECOMMISSION by end-2027 in the company governance catalog and its row carries `versions: []`, no evidence and no corpus - the same shape abinitio had before the SME email that C33 loads.** Source: the company-side session of 2026-09-07 (transcribed to internal-local/research/2026-09-07-c33-governance-catalog-mcp-capture.md, sections 2-3). The registry's informatica-powercenter row is a name with a vendor and nothing else; the governance read gives it a lifecycle phase and a date, which is exactly what turns a census from housekeeping into a MIGRATION INVENTORY (the C33 value observation, same source). What it needs is what abinitio got: a dated install census from the SME, captured as a corpus row in config/doc-source-registry.yaml with its capture rung, and then the C33 loader run against it - so C33's key decision (Q3) is made ONCE for both platforms and not re-asked per product. Do not mint a loader item for it: it is a C33 follow-up corpus, and C33's acceptance already forbids a second key. Also records the flag-not-build invariant the same session named: for a DIVEST/DECOMMISSION platform the fid population is CLOSED, so a fid appearing after the census date is a finding, not a row. Module drydocs-load (corpus registration + census), the invariant to drydocs-remediation as a detector candidate. Related [[C33]], [[Idea-98]], [[Idea-276]].

- **`Idea-274`** · 2026-09-07 · `[idea]` · **groomed → LOAD3 (2026-09-07)** · prio? **Med** — **Ten folder rows in `CM_DEF_VTAB` share a TABLE_ID WITHIN one data center, and the staging DDL's composite (DATA_CENTER, TABLE_ID) key would reject them - what a duplicate folder row means has never been asked.** Source: the P6 probe run (psgmgr replica, 2026-09-07; counts transcribed in the P6 item file): P6.0 read 76,753 folder rows against 76,743 distinct table ids while P6.1 found zero ids spanning two data centers, so the ten extra rows are same-id, same-data-center duplicates. P6 settled the cross-data-center question (no collision; folder_id is a safe graph key) and this is the question next to it: is a duplicate row a folder VERSION (the view carrying more than one version serial), a deleted-and-recreated folder that kept its number, or an artifact of the view definition? Each answer lands differently - a version row wants the newest kept and the rest dropped, a recreation wants the live one, a view artifact wants the view fixed. Probe first, counts only, on the P6 shape: group by (DATA_CENTER, TABLE_ID) having count > 1, then count distinct SCHED_TABLE, VERSION_SERIAL (or whatever the view exposes) and whether the rows differ at all. Then either the staging load dedupes by a rule the SME confirms, or the composite key is loosened - a HITL question, not a loader default. Module drydocs-load (staging DDL section 0.1 asked the P6 question; it should ask this one beside it).

- **`Idea-273`** · 2026-09-06 · `[idea]` · **groomed → CORE2 (2026-09-07)** · prio? **High** — **A source-scan absence guard needs a POSITIVE CONTROL: six of six absence scans across two Lane B bursts were vacuous on first write, because the stripper did not match the subject.** Source: the Lane B close report (laptop, 2026-09-06), process findings; WEB7's `path=` literal and O89's route literal (twice) this burst, three more in the 2026-09-05 burst. Every case is the same: `codeOnly` strips string literals, so a guard that asserts a LITERAL is absent passes on an empty scan; `withoutComments` is the stripper for a literal subject. Two were caught only by breaking the thing the guard claimed to catch, and one had a comment saying `withoutComments` beside code saying `codeOnly`. J66 says which helper to use; nothing makes the wrong choice fail. Mechanism: an `absent(pattern, files, *, positive_control=<snippet>)` helper in `tests/source_scan.py` that first runs the stripper over the control and refuses when the pattern no longer matches it - the mutation probe, built in, so a stripper/subject mismatch fails on the control and not silently on the tree. Sweep the existing absence guards onto it. Module drydocs-core (tests/source_scan.py).

- **`Idea-272`** · 2026-09-06 · `[adr]` · **groomed → DOC5 (2026-09-07)** · prio? **Low** — **WEB7's ADR candidate: code splitting follows the authorization boundary - the lazy-loaded route set IS the gated route set, derived from one table, never a second list.** Source: the Lane B close report (laptop, 2026-09-06), hand-back adr. The material is all present: WEB7's item note, the derivation guard that asserts the lazy set equals the gated set, the CI chunk ceiling (clause b), and the publish-boundary trace (clause c) showing every generated artifact is External / Internal-Public / `config/` tier - so the split stands on least privilege, not confidentiality, which deflates A4's devtools-leak claim and is the sentence the ADR exists to record. Draft PROPOSED, cite WEB7 and the guard by path, and settle in the same ADR whether the ungated lineage route's `load-map.json` import (which pins that artifact in the initial chunk) is an exception or a pending gate. Module adr.

- **`Idea-271`** · 2026-09-06 · `[idea]` · **groomed → LIN3 (2026-09-07)** · prio? **Med** — **Console feedback (`console.<route>-revN.yaml`, O89) loads and resolves to nothing, because no loader writes `:DocSection` rows for a `console.*` origin.** Source: the Lane B close report (laptop, 2026-09-06), hand-back lineage. O89 keys console feedback on content-derived anchors shared by the screen and paper halves, and the feedback loader already reports the unresolved case rather than inventing rows, so it degrades honestly - but the graph side of the loop is not closed until the docs/lineage loader emits section rows for console routes the way it does for design-doc sections. Decide the origin string (`console.<route-slug>`), what a console section IS (the anchored block, by its anchor), and whether the capture manifest `web/captures/capture-manifest.json` (machine-local, untracked) or the committed route table is the source of the row list; a machine-local file cannot be. Module drydocs-lineage.

- **`Idea-270`** · 2026-09-06 · `[bug]` · **groomed → API2 (2026-09-07)** · prio? **Med** — **`ColumnDef.type` is `'string' | 'int'` only, so two server columns that return LISTS are declared `string`: `runbooks.series.v1 lands` (`collect(DISTINCT d.assetId)`) and `lineage.schema-definition.v1 properties` (a Cypher list comprehension).** Source: the Lane B close report (laptop, 2026-09-06), hand-back api; verified at `drydocs_api/query_specs.py:73` (the vocabulary), `:1003` and `:1213` (the two declarations). WEB6's client-side column validation found them and exempts exactly those two by spec id in `web/src/data/rowShape.ts`, with a bidirectional guard in `rowShape.test.ts` that fails when the server is fixed AND when a third one appears - so this is not a two-string edit. The fix is a vocabulary extension: a list column type in `ColumnDef` (and its OpenAPI shape, which moves `openapi.json` and `api.d.ts`), the two declarations moved onto it, the client validator taught the new type, the exemption deleted, and a server-side guard that a column whose Cypher collects or comprehends is not declared scalar (read the spec object, J37). R15/R16 own the api package, so land it with or right after them, not before. Module drydocs-api.

- **`Idea-269`** · 2026-09-06 · `[skill]` · **groomed → PLAN5 (2026-09-07; the outputs-versus-convention RULING in clause (b) stays the user's and is carried on the item file)** · prio? **Med** — **`lane-handoff --suggest` compares a Lane B item's inputs against Lane A's PENS and never against Lane A's QUEUE, so four of ten queued items passed clean while sharing input paths with items Lane A was building.** Source: the Lane B close report (laptop, 2026-09-06), headline finding: WEB8, O63, R12 and O68 all passed `--suggest`; WEB8 relocates `askApi.ts` while WEB9 (Lane A's queue) edits it, O63 rewrites the same file, and both need `drydocs_api/schemas.py` under R15/R16's whole-package input. Lane B skipped WEB8 and O63 by hand and took R12 and O68 knowingly. The check that was missing is item-inputs against other-queue item-inputs, prefix-aware so `drydocs_api/` covers `drydocs_api/schemas.py`; `--other-queue` already carries the ids, so the data is in hand and the output is one row per collision naming both items and the shared prefix. Lane B's ~30-line version was recovered and preserved AS RUN at `.claude/skills/lane-handoff/scripts/overlap_prototype.py` (`e89a66ca`, merged `47383aae`): start from that file, not from this description; it retires when the fold-in lands in `handoff.py`. Three facts the fold-in needs, found by re-running it (2026-09-06): (1) run cold it reports SEVEN of the ten, not four - WEB2, WEB6 and WEB7 collide only on `docs/reviews/modules/web-2026-09-05.md`, the review that spawned every WEB item on both lanes, which is read-only provenance and not a write target; a shared input is a collision only if one side WRITES it, and that is not expressible today because `inputs` is the only path field in the v3 schema and no item declares `outputs` - so it is a RULING: grow the schema, or exclude provenance by convention (`docs/reviews/`), and 66 of 663 items name a review path, so the choice is not cosmetic; (2) `--check` cannot do it without a front-matter change - a generated handoff records `queue:` and `pens:` and the other lane's ids survive only in a prose table row, which J37 forbids parsing, so generate writes an additive `other_queue:` line and `--check` takes a graceful path for files that predate it (`docs/lane-b-handoff.md` is one); (3) the prefix rule is symmetric so a coarse input dominates - O68's `web/src` alone matches seven Lane A paths; one row per collision, and the row says which side was coarse. Fold into `--suggest` and `--check`, pin in `tests/unit/test_lane_handoff.py`. Module drydocs-plan.

- **`Idea-268`** · 2026-09-05 · `[bug]` · **groomed → API3 (2026-09-07)** · prio? **Med** — **`scripts/dump_openapi.py` opens `web/src/generated/openapi.json` for writing BEFORE it calls `render_schema()`, so a render failure leaves the committed artifact at zero bytes.** Source: the Lane B close report on API1 (laptop, 2026-09-05), hand-back one. A generated file that is truncated by its own writer's failure reads as a corrupt commit rather than a failed run, and `npm run api:types` then fails on an empty document one step later with no pointer back. Fix is two lines: render to a string first, then open and write; and exit non-zero with the exception named. Add the failure case to the test that pins the writer. Module drydocs-api.

- **`Idea-267`** · 2026-09-05 · `[docs]` · **groomed → WEB16 (2026-09-07)** · prio? **Low** — **`npm run api:types` needs `DRYDOCS_DATA_ROOT` set and says so only as a Python traceback buried inside a Node stack.** Source: the Lane B close report on API1 (laptop, 2026-09-05), hand-back two; the same wall was hit again at the Lane A merge. The script shells out to `scripts/dump_openapi.py`, which imports settings that resolve the data root, so an unset root surfaces three layers down. Either the npm script checks the variable first and prints the one-line requirement, or the web README's generated-artifacts section names it beside the command (it names the command today and not the precondition). Module drydocs-web.

- **`Idea-266`** · 2026-09-05 · `[chore]` · **groomed → WEB17 (2026-09-07)** · prio? **Low** — **Two web build follow-ups from the Lane B stack, both one-liners with a decision attached.** (1) WEB4's strict tsconfig excludes the playwright and vitest config files from `include`, so they are type-checked by nothing; widening `include` is one line and the WEB4 close note left it to Lane A to mint or drop. (2) WEB13's `test:coverage` writes `web/coverage/` and nothing ignored it - 184 report files rode into the WEB13 feat commit and were untracked at the merge with a `coverage` line in `web/.gitignore` (`aa7e5836`). Nothing left to do on (2) unless the coverage HTML should be published somewhere, which is the decision. Source: the Lane B close reports (laptop, 2026-09-05) and the Lane A merge. Module drydocs-web.

- **`Idea-265`** · 2026-09-05 · `[port]` · **groomed → PORT5 (2026-09-07)** · prio? **Med** — **A PORT-MANIFEST `entry_rule` that describes the CONSUMER's tree ("company = producer MINUS x") is stale the moment the consumer moves, and only the consumer can see that it did.** Source: the company's close-out on port-base-20260902 (transcribed machine-local, 2026-09-05): the `config/taxonomy/ui-components.yaml` rule encoded the K7-K15 hold as "producer minus the AppCodeCascadePane row" while the company had carried the row since before carve-out 8, so honoring the rule would have failed three of its guards. The manifest can only know the producer's tree. Sweep every `entry_rule` for a consumer-tree assertion (a hold, a "they do not have", a fixed company count), rewrite each as a rows-union rule plus a TRACKED per-side count, and add a `test_port_manifest.py` check that an `entry_rule` never contains "company = producer MINUS" or a bare company count. Where a hold is real it belongs in the company's own divergence ledger, which the company maintains. Module port (PORT-MANIFEST.yaml).

- **`Idea-264`** · 2026-09-05 · `[skill]` · **groomed → PLAN6 (2026-09-07)** · prio? **Med** — **`lane-handoff --suggest`'s `V` (venue-bound) marker reads only an item's `inputs` paths, so a venue wall written in acceptance prose - G132 (g)'s "none of which exist producer-side" - is invisible to the check.** Source: the Lane B close report (laptop, 2026-09-05): G132 was queued to the laptop and hit the wall on read; the lane left it `todo` rather than claim what only a server-side machine can build. The fix is not to parse prose: give items a declared `venue:` (or a `requires:` list) the check can read, and have `--suggest` flag any queued item with a venue the receiving machine does not declare in `config/dev-environment.yaml`. Module drydocs-plan.

- **`Idea-263`** · 2026-09-05 · `[config]` · **groomed → CFG3 (2026-09-07)** · prio? **Low** — **`precedence.schema.json` and `relationship-vocabulary.schema.json` have no top-level `additionalProperties: false`, so either accepts any stray key silently.** Source: the Lane B close report (laptop, 2026-09-05), J58: three family schemas (data-centers, domains, editions) are closed at the top level and that closure is what settled the identity-header profile split; these two are the open ones left. Closing them is one line each plus a check that nothing in the tree already relies on a stray key. Module config.

- **`Idea-262`** · 2026-09-05 · `[ci]` · **groomed → CORE3 (2026-09-07)** · prio? **Med** — **CI clones shallow (`actions/checkout` with no `fetch-depth`), so the J74 commit-message ceiling guard detects the shallow clone and skips loudly instead of measuring; `fetch-depth: 0` would let it run there.** Source: the Lane B close report (laptop, 2026-09-05), J74 (d): the guard was built to refuse a false zero on a shallow history. Cost is a full-history clone per run; the repo is 2,057 commits, so small. Decide once and record it in the workflow file's comment. Module drydocs-core (CI).

- **`Idea-261`** · 2026-09-05 · `[docs]` · **merged → G132 (2026-09-07) as a second reason behind clause (e), the directory-index question** · prio? **Low** — **`collect/` now holds two collectors (rua inventory and the Z4 load-balancer resolver) under a README titled for the first; G132 (e)'s directory-index question has a second reason behind it.** Source: the Lane B close report (laptop, 2026-09-05), Z4: the new collector's README sits beside the rua one rather than rewriting it, because G132 (e) owns the index. When G132 runs, the index should name both and the per-collector READMEs stay. Module drydocs-lineage.

- **`Idea-260`** · 2026-09-05 · `[docs]` · **groomed → DOC6 (2026-09-07)** · prio? **Low** — **The bundled demo's verb sequence - which commands, in which order, against which database - is written down nowhere a newcomer would find it.** Source: the Lane B close report (laptop, 2026-09-05): reproducing Z8/Z7 live on a fresh `z8demo` database meant reconstructing the chain from item notes. `internal/repo-README.md` already holds the runnable pipeline and is where the sequence belongs; sample-reproducible, so it also serves as the J18 evidence recipe. Module docs.

- **`Idea-259`** · 2026-09-05 · `[fixtures]` · **groomed → LOAD4 (2026-09-07)** · prio? **Med** — **Only one bundled folder can ever attribute: the SEAL capture declares applications 70001-70003 while the sample folders carry 70002, 70011, 70012, 70021, 70022, 70031 and 70041.** Source: the Lane B close report (laptop, 2026-09-05), Z7: the demo's coverage counters show 7 of 8 folders unattributed, and this is why - the fixture's application ids and the folder ids were authored apart. Either the capture grows the six synthetic apps or the folders narrow to the three; the gaps kept live on purpose (2 hosts unresolved) are a separate, deliberate set and should stay. Module drydocs-load.

- **`Idea-258`** · 2026-09-05 · `[tests]` · **groomed → CORE4 (2026-09-07)** · prio? **Med** — **J8's skip guard treats any `drydocs/data/` reference as gitignored, but the sample CSVs under it are tracked - `git ls-files` tells the two apart.** Source: the Lane B close report (laptop, 2026-09-05), found while moving the Z8 fixtures: the file-level policy flagged a reference to a tracked sample as if it were machine-local data, and the fix on Z8 was a real existence guard rather than a fake skip. The policy's test should ask git whether the path is tracked, not pattern-match the directory. Module drydocs-core (tests).

- **`Idea-257`** · 2026-09-05 · `[plan]` · **open - re-read at the 2026-09-07 groom and NOT groomed: the entry says itself that what "subject area" refers to is not settled and should be ASKED, and the two candidates (an edition, a domain) land in different registries. A question for the user, not a guess** · prio? **Med** —
  **The Idea grammar is uniform across editions, but what each subject area DOES with its inbox may need re-tooling to its business — the SME's note at the idea-series-grammar signing, captured for grooming rather than folded into the ruling.** Source: gate `idea-series-grammar` SIGNED 2026-09-05, §B1, SME verbatim: *"Idea series take the same edition segment as items, each subject area might need some re-tooling based to business."* The record takes the first clause as the ruling (`[<EDITION>-]Idea-<n>`, one segment rule for both id kinds) and this entry holds the second, because a note inside a signed clause reads as a condition and this was not one. What "subject area" refers to is NOT settled here and should be asked, not assumed: the two candidates in the tree are an EDITION (an Area Product — the id-space partition, editions.yaml) and a DOMAIN (a vocabulary partition, domains.yaml), and §A2 of the parent record says the two registries never share. If it means editions: an instance's inbox is already its own (DOC1), so "re-tooling" would be about capture practice — what an Area Product's support team writes down, in what shape, with what triage — and belongs with the Teams Edition template's instance-owned inbox (ADR 0015 D2 as amended), not the allocator. If it means domains: the inbox has no domain axis today (Idea entries carry a `[module]` tag, not a domain), and adding one is the same question C41 §B1 answered for items with `domain:` on the item file — an additive field, validated against domains.yaml, never a segment in the id. Either way the grammar does not move; the SME's phrase "based to business" points at the capture PRACTICE varying per area while the id SHAPE stays one. Groom after PLAN2 (b) lands, when the segment exists to hang the question on. Related: PLAN2, PLAN4, DOC1, CFG1, CFG2, [[Idea-254]], [[Idea-256]].

- **`Idea-256`** · 2026-09-04 · `[taxonomy]` · **groomed → ONT3 (2026-09-07), the GATE-PROMPT half only - admitting the Logical Container tier is the SME's and routes through the gate; findings 1, 2 and 4 need no ruling and ride the prompt** · prio? **Med** —
  **A five-tier naming formula `[LOB]_[PROD_LINE]_[FUNC_LAYER]_[LOGICAL_CONT]_[APP]` proposes one tier the declared taxonomy has no home for - the Logical Container (data product) - and renames the tier the edition grain is cut at.** Source: the SME's synthetic hierarchy sketch, 2026-09-04 (transcribed here in full because the sketch file was deleted after capture; nothing external is cited). The formula is for system names, tags and database schemas, with three synthetic worked examples (Retail/Mortgage, Wealth/Brokerage, Commercial/Treasury) whose codes - RET, WLT, COM, MTG, CLOS, AVE - collide with none of the declared LOB codes (CCB, AWM, CIB, CT, HR, ET), so it is genuinely invented and carries no real estate values. Four findings. (1) IT IS A DIFFERENT AXIS, NOT A REPLACEMENT: `config/taxonomy/lob-product-team.yaml` is CatalogLOB > ProductLine > Product > DevTeam and ends at the org unit (`not_concepts: [dev_teams]` - never a subject); this formula ends at the APPLICATION and carries no DevTeam. It is an addressing/naming key that shares the top two tiers with the org taxonomy, so it conflicts with nothing there. (2) THE EDITION GRAIN SURVIVES, and this is the part worth being explicit about: `FUNC_LAYER` reads as the Area Product. Gate ontology-domain-registry-and-edition-grain SC2 defines one as "where two or three applications are delivering on the same business topic", and the sketch's own example meets it exactly - Loan Origination & Underwriting holds CLOS and AVE, two applications on one topic. SC2 also says the definition qualifies a row, "not the Product Catalog tier name by itself", so a tier LABELLED Functional Layer still cuts editions at the same grain: no re-mint, no new ruling needed. (3) THE ONE REAL GAP - the Logical Container (data product: Applicant Credit Assessment, Real-Time Equities Positions, Intraday Liquidity Balances) sits between the Area Product and the application and has NO tier in the declared taxonomy. It is not the registry's `dataset` (that grammar is physical, `origin@db.schema.table`) and not `:DataAsset` (the D1 proxy URN, also physical); a curated logical container is what DPROD/EKGF is for, and DPROD is already registered under `reference/standards/` and instantiated nowhere. Admitting the tier is an ontology decision and goes to the HITL gate, never into an import (CLAUDE.md S1). (4) A CAUTION IF THE FORMULA EVER BECOMES A KEY: `[APP]` is described as the application ACRONYM, and applications are keyed here on app_id / SEAL id. An acronym is a label; identity is a business key and the composite string would be a render of it (the SG1 rule the registry already applies to URNs). Module config / drydocs-core for any reader; the DPROD question is ontology-mapper's.


- **`Idea-255`** · 2026-09-04 · `[idea]` · **open - re-read at the 2026-09-07 groom and NOT groomed: the harvest itself is company-side acquisition and the producer-side reader waits on the sosa-jobrun-observation gate, so there is no pass/fail acceptance to write here yet** · prio? **High** —
  **Run-history harvest: the resolved command line per run, from the audit table and a daily sysout harvest, as an Oracle daily-history view feeding second-pass job properties (SOSA).** Source: the user's concept, 2026-09-04 (chat; nothing committed before this entry). The definition pass proves configuration; the runtime pass needs a witness per run, and the witness exists twice in the estate: the scheduler's audit/history table (a year of runs, reruns included) and the per-run sysout. Harvest both daily into one keyed history - `<server, tenant, job, order id, odate, run#>` - carrying the resolved command line as executed, and expose it as a daily Oracle history view the second pass reads (docs/reviews/job-etl-binding-second-pass-options-2026-09-04.md, the runtime class; this harvest is the missing evidence the `sosa-jobrun-observation` gate prompt has been waiting for). Points to carry, all mechanism: (1) the vendor corpus has NO sysout directory or file-name pattern - the route is the vendor's own retrieval verb (research Q28), and Output Handling do-actions mean per-job RELOCATION, so the file LIST is the denominator, never a directory walk; (2) two distinct nulls, never one - host-not-in-scope (the harvest never reached it) versus harvested-and-no-log (it reached it and found nothing), counted apart (G11); (3) CM_HOSTS grain is the host GROUP, and the server name in the key resolves the group to the member (Z4's ground truth); (4) a drift alert between the definition and the executed line still needs the resolver - `{ODATE}`-class residue is normalized out or the alert fires nightly; (5) redact BEFORE anything leaves the host (bearer tokens in banners - MM7's redaction rule), so an emailed summary never carries one; (6) phase 2 widens config/launcher-registry.yaml from what the harvest shows launching; (7) the heat map is K25's grid, rendered company-side only - the rows are tenant ids, Internal. Module drydocs-lineage for the reader; the harvest itself is company-side acquisition on the Idea-133 shape (mode, via).

- **`Idea-254`** · 2026-09-04 · `[chore]` · **closed 2026-09-04 — built directly, no item minted (desktop): `as_dict()` serializes `clone_pipeline_guids_distinct`; the list stays on the object where staging's cross-check reads it, and `clone_missing_set_guids` stays a list because it is the work list** · prio? **Low** —
  **The staged lineage artifact serializes every clone pipeline GUID the MAC extractor parsed.** Source: the LIN1 verify pass at 93f4d832 (laptop, 2026-09-04), follow-up (b). `MacCoverage.clone_pipeline_guids` (dpl_mac.py:192) was added at the LIN1 review so staging would not re-walk the tree, and `as_dict()` now carries the whole list into the artifact's coverage block - at estate scale that is one line per pipeline folder in a header meant to be read. Keep the list on the coverage object for staging's cross-check; serialize a COUNT (and the mismatches, which are the finding) into the artifact. The other two follow-ups (dirty-tree code_commit, unguarded zone ids) went into LIN2's acceptance because they bite at load time. Module drydocs-lineage.

- **`Idea-253`** · 2026-09-04 · `[guard]` · **groomed → PORT4 (2026-09-07)** · prio? **Med** —
  **The port drop guard has no accepted-drop seam.** Source: the company's chunk-5 apply of port-base-20260902 (their idea inbox, relayed by the SME 2026-09-04; the producer holds no transcript, so traceability to their id is one-directional). The file-level drop guard reports every path a take would remove and has no way to record that a specific drop was RULED, so a retirement the port already decided is reported again at every chunk and reads like a finding. The precedent is one level up: `drydocs/port/port_backlog_union.py` keeps `UNION_EXCLUSIONS` - an accepted difference rendered under its own heading, never as silence and never as a failure, with a stale allowance failing the check. The seam for files is the same shape: an `accepted_drops:` block in the side-local overlay (`PORT-MANIFEST.*.yaml`, the J34 slot - each side's own rulings, never carried across), keyed by path with the ruling's provenance (report id, date), read by the guard so an accepted drop is listed as ruled and a stale allowance (the path exists again) fails. Its two siblings from the same apply landed directly: the lane-handoff skill's `row_may_match_nothing` entry and `drydocs_core/component_map.py`'s per-entry row (this commit). Module drydocs-port.

- **`Idea-252`** · 2026-09-03 · `[guard]` · **groomed → PORT3 (2026-09-07)** · prio? **Med** —
  **A canonical-producer test that pins a never-port path fails on every consumer tree by construction, and nothing checks the join.** Source: the company's chunk-2 apply of port-base-20260902 - one retired stub (`docs/port/port-prompt.md` on their side) broke three separate producer guards: the `test_markdown_fences.py` canary, the fence-scope change, and all four tests of `test_runbook_currency.py` through `EXTRA_DOCS`. Each was fixed producer-side after the fact (`20449d47`, and the `NEVER_PORT_ZONE_OF` skip), and each was found by the company running the full suite against a baseline rather than by anything on this side. The pattern is one guard: for every test whose manifest disposition is `canonical-producer`, read its CODE (J66 - `tests/source_scan.py`, string literals that look like repo paths) and assert no literal lies under a `never-port` row of `PORT-MANIFEST.yaml` unless the test names it in an exemption that says how it skips when absent. Reads the importable manifest and the code, never the prose (J37/J66). And the exemption's skip must be TRACKED-content-based, never a directory probe: the company's `docs/port/` exists and holds one gitignored file, so `is_dir()` read a zone that never crossed as present (found the same day, fixed with `git ls-files`) - the guard wants to catch a cited path that is present-but-untracked as well as one that is absent. Module drydocs-port.

- **`Idea-251`** · 2026-09-03 · `[idea]` · **groomed → PORT2 (2026-09-07)** · prio? **Low** —
  **A HITL feedback capture whose rev1 and rev11 normalize to the same anchor set is either an unfilled template or a normalizer that discards the payload.** Source: the company's chunk-1 rename-look on port-base-20260902 (their PORT-REPORT, question 7): `docs/design/feedback/drydocs-startup-refresh-runbook-rev11.yaml` and `rev1.yaml` score 1.00 text similarity after the detector's normalization. Not a port question - a J72 measurement artifact - but for a feedback file keyed on anchors (L5/L6) it means the two revisions differ only in payload the normalizer strips, or not at all. Check which: if the payload is the difference, the detector's `normalized_text` is too aggressive for feedback YAML (a separate class from prose and fragments); if the files are the same, rev11 was captured empty. Module drydocs-docgen / drydocs-port.

- **`Idea-250`** · 2026-09-02 · `[port]` · **groomed → PORT1 (2026-09-07), clauses 1-3 plus the workplan rule; finding 4 resolved itself the same day and is recorded on the item for the record** · prio? **Med** —
  **Three back-flow candidates from the company's chunk-1 apply of port-base-20260901, and one classification question the company's record must answer.** Source: the company session's chunk-1 status as relayed by the SME on 2026-09-02 (their record; producer holds no transcript). (1) `scripts/render_port_dispositions.py` should take an optional HEAD ref - the consumer's HEAD is its own tree, so a range on their side classifies tag..tag, never tag..HEAD; they patched `main()`/`render()` for it, adopt the shape. (2) The fourth guard in `tests/unit/test_port_dispositions.py` (the APPLY-BY-DISPOSITION section sits above STEP LEDGER in the port-prompt) is producer-only by nature - the company's port-prompt copy is retired and carries neither marker - so it should SKIP when the markers are absent rather than force the consumer to drop a test. (3) `scripts/port_rename_check.py` spawns one `git show` per producer file over the whole tree; `git cat-file --batch` on one pipe would turn minutes into seconds, and the whole-tree look is the one RELAY-20 asks for before every clean-add slice. (4) RESOLVED the same day, in the company's favor - the producer's 519 was measured ..HEAD three commits past the tag; tag..tag with the manifest at the tag is 516 / 130 / 150 / 4, and their 123 is 130 minus the seven derived rows. Kept for the record: their class table read 516 paths against the producer's 519, `canonical-producer` 129 -> 123, `per-entry` 154 -> 150, `derived` 4 -> 11; the seven `web/src/generated/**` re-dispositions explain +7 on derived and nothing else - three paths are absent from their range and at least one moved per-entry -> canonical-producer, which is the J68 shape. The likely cause is classifying with THEIR manifest rather than the manifest at `port-base-20260901` (the manifest is in the range). The plan's chunk 1 should say: classify with `git show <base>:PORT-MANIFEST.yaml`, then diff the two manifests - the differences are the port's own manifest rows. Also: the plan told them to run a renderer that landed AFTER their base (J69 at `bed22549`) - a workplan that names a script should name the commit it needs. Module drydocs-port.

- **`Idea-243`** · 2026-09-02 · `[chore]` · **open** · prio? **High** —
  **The metrics ledger records a STALE graph 51 rows in 52 - measure the snapshot the ritual just wrote, add A2, and break the data_root cycle it could not see.** Source: the 2026-09-02 tech-debt review (`docs/reviews/tech-debt-pre-restructure-2026-09-02.md`). P1 + C1 + C3 + T6. `snapshot.ps1` writes a snapshot, then `metrics_ledger.py` (:106-110) measures the LIVE graph nothing has loaded, so its A3 top module reads `loaders/base.py` = 37 while the fresh graph says `repo_paths.py` = 43, and its A5 reads 31 against 27. Compute A3/A4/A5 from the snapshot JSON (the skill's offline fallback already says how; the snapshot IS the live tree) and keep the live-graph column as the drift comparison; add `a2_circular` from `stats.circular_files`, because A2 moved 0 -> 5 at G81 (`701e1d22`, 08-23) with no record: `data_root.py:142` -> `data_zones.py:44` -> `data_root`, and `data_zones.py:237` -> `landing_zones.py:46` -> `data_root`, held apart by function-level imports. Same commit family: `_str_or_none` x5 in `drydocs_core/models/` and `_git` x5 -> core (MODULE_MAP 'Future, land in core'); drop the `SPIDERP_LOGDIR` alias, whose ADR 0014 clause-1 trigger (the next port) passed on 08-26. Module drydocs-load for the ledger, drydocs-core for the rest.

- **`Idea-244`** · 2026-09-02 · `[chore]` · **groomed → ADR 0018 + CORE1 + DOC3 (2026-09-02)** · prio? **High** —
  **Every boundary component group maps to a modules.yaml module - add drydocs-port and its PORT series - and the five unrouted top-level directories get a class or a retirement.** Source: the 2026-09-02 tech-debt review (`docs/reviews/tech-debt-pre-restructure-2026-09-02.md`). A1a + A3. `test_module_boundary.py` COMPONENT_GROUPS has a `port` group (`port_preflight`, `port_backlog_union`, `port_rename_detect`) and `modules.yaml` has no `drydocs-port`, so port work has no id series under PLAN1 and nothing guards the two lists against each other. Add the module + `PORT` series, and a guard: keys(COMPONENT_GROUPS) <= modules.yaml (with the declared exceptions `agents`, `libs`). Then `SDLC-Docs/`, `graph-tests/`, `drydocs-icons/`, `libs/`, `knowledge/upgrade-plans/` - named in no routing document - each get a MODULE_MAP 'Non-Python surfaces' row or a retirement (`SDLC-Docs/` is the 2018 genesis document; register it under `reference/` with a classification, or move to `docs/history/`). ADR 0015's copier will need a file class for every path; 'nobody knows' is the class they get today. Module config / docs. The subpackage question (A1b) is NOT this - it is an ADR 0015 D2 ruling carried by DOC1, and files do not move before it.

- **`Idea-245`** · 2026-09-02 · `[chore]` · **open** · prio? **High** —
  **CI tests one Python and the desk runs another - a 3.11/3.12/3.14 matrix, per-job timeouts, a pre-commit ruff hook, a snyk-skipped notice; then the dependency cluster in blast-radius order.** Source: the 2026-09-02 tech-debt review (`docs/reviews/tech-debt-pre-restructure-2026-09-02.md`). D3 + P5 + D1 + D2 + D4. `pyproject.toml:11` says `^3.11`, `ci.yml` tests 3.12 only, this desktop runs 3.14.6 (and sees neo4j's asyncio deprecation CI never sees). Matrix on the `gates` job, fail-fast off; `timeout-minutes: 20` on all three jobs; `.pre-commit-config.yaml` with the two ruff hooks (the mechanism behind Idea-111's 100-run red streak); one line when `snyk` skipped for a missing token. Then, staged: pytest 8->9 + pytest-cov 5->7 (test-only); rich 13->15 + typer 0.12->0.27 together (the CLI); neo4j 5->6 last and behind an integration run; oracledb 2->4 when a live connection can prove it. The npm side needs a DECISION, not a command: 5 high advisories (`js-cookie` <= 3.0.5, GHSA-qjx8-664m-686j) via `@neo4j-nvl/react` -> `@neo4j-nvl/base` -> `@segment/analytics-next`, and the only `audit fix` is a breaking NVL downgrade - try an `overrides` pin first; and both `@neo4j-nvl` packages are MISSING from the local `node_modules` (declared, not installed). `poetry check` also wants PEP 621 `[project]` metadata (six deprecations). Module graph-infra / drydocs-web.

- **`Idea-246`** · 2026-09-02 · `[chore]` · **open** · prio? **Med** —
  **Coverage is 90% overall and 12-28% in the CLI - configure and REPORT it, split the 120-line commands into pure halves, run the integration tier nightly, and decide mypy.** Source: the 2026-09-02 tech-debt review (`docs/reviews/tech-debt-pre-restructure-2026-09-02.md`). T2 + C2 + T4 + T3 + T5. `pytest-cov` is a dev dep with no `[tool.coverage]` block and no CI step; measured 19,083 stmts / 1,939 missed, and the six S8 command modules plus `cli.py` are 42% of every miss (`cli_variables` 12%, `cli_verify` 14%, `cli_docs` 22%, `cli.py` 28%). A5 agrees: 27 modules no test imports, five of them the CLI. Report first, threshold later (a threshold set today is a number nobody chose). `create_app` is 625 lines (`drydocs_api/app.py`); 18 functions are 120+. `addopts` deselects `tests/integration/` and CI runs bare pytest, so the five testcontainers modules run on a laptop on demand - a nightly `integration` job with Docker. `mypy ^1.10` is declared and has never run (`pyproject.toml:34` is its only mention): remove it or scope `[tool.mypy]` to `drydocs_core` and grow outward. And the worktree fixture in `test_repo_paths.py` leaked a LOCKED pytest worktree (pruned with the review) - unlock in a `finally` before removing. Module drydocs-load / drydocs-api / graph-infra.

- **`Idea-247`** · 2026-09-02 · `[docs]` · **groomed → ADR 0018 + DOC3 (2026-09-02; Doc3's 79 broken links and Doc4's MODULE_MAP size stay open, inbox them again if they matter)** · prio? **High** —
  **git-readme.md names 31 paths that moved at the 0002-A extraction and the narrative plan stops at phase 5 of 18 - sweep both and add a routing-doc path guard.** Source: the 2026-09-02 tech-debt review (`docs/reviews/tech-debt-pre-restructure-2026-09-02.md`). Doc1 + Doc2 + Doc4 + Doc3. `git-readme.md`: `drydocs/controlm/*.py`, `drydocs/models/*.py`, `drydocs/schema/*.cypher`, `drydocs/ontology/*` (all now `drydocs_core/`), `config/taxonomy-ontology-map.yaml` and `relationship_vocabulary.yaml` (both now directories), and four more; it is the port's routing doc, and PLAN2 already edits its :196-201. `MODULE_MAP.md` cites seven moved paths; `01-project-plan.md` (07-22) documents phases 0-5 while `plan.yaml` runs to 17, and CLAUDE.md's last line sends readers to it for 'the plan' - render the phase narrative from `plan.yaml` or cut the doc to what YAML cannot hold. Guard on the L19 pattern: every backticked repo-relative path in the four routing docs (`CLAUDE.md`, `MODULE_MAP.md`, `git-readme.md`, `internal/repo-README.md`) resolves; warn-only for the other 79 broken relative links in 38 files (11 vendored skills link a `CONNECTORS.md` that never existed; 0 publish-boundary leaks). Caveat for any date-based staleness metric: history is squashed to 07-20, so nothing can measure older than 44 days. Module docs.

- **`Idea-248`** · 2026-09-02 · `[chore]` · **open** · prio? **Med** —
  **Fourteen gate prompts await sign-off and ~40 prompt files have no shape test; the Ready strip is 85% of the backlog and five finished epics are still open files.** Source: the 2026-09-02 tech-debt review (`docs/reviews/tech-debt-pre-restructure-2026-09-02.md`). P3 + P4. 58 prompts in `config/gate-prompts/`, 96 records in `config/gate-log.md`, 14 stems with zero log mentions (`bmc-docs-example`, `business-layer-org-structure`, `code-graph-package-layer`, `console-auth-boundary`, `controlm-definition-precedence`, `data-flow-overview`, `dcat-theme-subject-scheme`, `document-supersession`, `dpl-dataset-identity-zone`, `dpl-dataset-registry-contract`, `dpl-pipeline-registry-contract`, `registry-wiring-readiness`, `repo-manifest-data-profile`, `script-provenance-gaps`); 61 of 104 `config/**/*.y*ml` have no test reference, mostly prompts, so a malformed prompt fails at the SME session. One shape test over the fields the gate-page renderer reads, and the unsigned list on the queue page (the derived list under-reports RECORD entries - the 2026-08-31 finding). `next_ready = 124` of `todo = 146`: dependency edges do no sequencing, so a grooming pass per epic declares the real order (the eight restructure items are the model). Close `cdo-alignment`, `ddlineage-retirement`, `orchestrator-expansion`, `provenance-audit`, `taxonomy-capture` (all items done). Module docs / config.
...
- 2026-09-08 - [groom] Idea-303 -> PLAN10.
- 2026-09-08 - [groom] Idea-302 -> PORT7.
- 2026-09-08 - [groom] Idea-301 -> DOC7.
- 2026-09-08 - [groom] Idea-299 -> REM2.
- 2026-09-08 - [groom] Idea-297 -> DOC8.
- 2026-09-08 - [groom] Idea-296 -> CORE9.
- 2026-09-08 - [groom] Idea-295 -> LOAD7.
- 2026-09-08 - [groom] Idea-287 -> REV1.
- 2026-09-08 - [groom] Idea-286 -> PLAN11.
- 2026-09-08 - [groom] Idea-285 -> DOC9.
- 2026-09-08 - [groom] Idea-300 -> merged into PLAN8.
- 2026-09-08 - [groom] Idea-289 -> merged into G136.
