...
- **`Idea-283`** · 2026-09-07 · `[bug]` · **open** · prio? **Med** — **The `test_recursive_sql_cyclic_type_disabled` guard asserts a comment phrase, not the absence of the cyclic-type join from executable SQL - reword the comment and it fails, re-enable the join and it passes (J66).** Body follows.

- **`Idea-282`** · 2026-09-07 · `[idea]` · **open** · prio? **Med** — **Term search over `Chunk.text` and a chunk-level trust override are the two follow-ups the cyclic-type-trap eval case named and never inboxed; `pipeline.py` hardcodes `chunks: 0`, so no /ask tier reads chunk text today.** Body follows.

- **`Idea-281`** · 2026-09-07 · `[idea]` · **open** · prio? **Low** — **An eval case that pins a sha should say which of its sections are sha-sensitive: the two research-probe cases are 367 commits behind on exactly the console-arm findings that moved.** Body follows.

- **`Idea-280`** · 2026-09-07 · `[idea]` · **open** · prio? **Low** — **`R10` and `R1` name two unrelated things - a remediation rules-registry rule and a backlog item - so a bare grep lands on the wrong artifact; cite registry rules as `registry R10` on first use.** Body follows.

- **`Idea-279`** · 2026-09-07 · `[idea]` · **open** · prio? **Med** — **The /ask scope selector is a ROUTER HINT that filters the spec catalog at `pipeline._route`, not a second retrieval path - and its vendor-corpus option must not ship until a spec searches chunk text.** Body follows.

- **`Idea-278`** · 2026-09-07 · `[idea]` · **open** · prio? **Med** — **A synonym -> canonical term -> home ledger for Control-M knowledge, filled into the research-terms shell that already exists (three verified seed rows), instead of a fifth skill.** Body follows.

- **`Idea-277`** · 2026-09-07 · `[idea]` · **open** · prio? **Med** — **The Explorer header puts the word Tower beside `backs onto drydocs` - the console half of the R22 provenance fix is missing; a hover sourced from `ui-concepts.yaml`, never hand-written, with one open question (generated json or live spec).** Body follows.
