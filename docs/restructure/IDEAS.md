...
- **`Idea-306`** · 2026-09-08 · `[idea]` · **open** · prio? **Med** — **A behavior switch in `config/dev-environment.yaml` that only a test reads is a mechanism whose guard and whose behavior disagree about the same fact; every switch there needs a non-test consumer, or a stated reason it has none.** (Stub: id minted and pushed first, per I6; body follows.) Module config. Related [[J76]], [[J78]].
