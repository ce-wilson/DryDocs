# Port prompt ARCHIVE — steps 1–42 (through producer head `6fd3270`, APPLIED)

> **FROZEN 2026-07-21.** Everything in this file was applied company-side by
> PORT-REPORT-6fd3270 (branch drydocs-port-20260721). The live prompt is now the
> rolling [`docs/port-prompt.md`](port-prompt.md) — durable guardrails + only the
> delta since `6fd3270`, with step numbering continuing at 43. External references
> to "port-prompt step N" for N ≤ 42 resolve HERE; do not renumber. Do not extend
> this file — new steps go in the rolling prompt.
