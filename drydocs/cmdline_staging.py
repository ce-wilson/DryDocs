...
``drydocs export-cmdline-staging`` (G39)
    ...
    source for the same shape. EXPLICITLY OUT OF SCOPE, by design not
    omission: folder/job VARIABLE columns — the variables pull stays
    deferred exactly as the original G18→G22 plan sequenced it.
...
``drydocs parse-cmdline-staging`` (G40)
    ...
    — into structured detail columns beside the G39 rows. Unparseable /
    partial rows go to the WARN stream and are COUNTED, never dropped; a
    parsed/partial/unparsed coverage line prints on every run.
...
SCHEMA_VERSION = "drydocs.cmdline-staging.v1"
...
-- G39: one row per loaded job. VERBATIM cmd_line; variables OUT OF SCOPE.
...
def export_job_detail(client, db_path: str | Path) -> ExportReport:
    """Materialize the G39 store from the graph. The job_detail table is
    REPLACED (the graph is the truth; the store is derived and rebuildable);
    parsed tables are cleared too — they describe the replaced rows."""
        ...
        with conn:
            ...
            conn.executemany(
                ...
                sorted({
                    ...
                    "variables": "OUT OF SCOPE by design — the folder/job "
                                 "variables pull stays deferred (G18->G22 plan)",
                }.items()),
            )
    finally:
        conn.close()
...
def parse_job_detail(db_path: str | Path) -> ParseCoverage:
    ...
    never on a variable's name. NO graph writes.
    """
    db_path = Path(db_path)
    ...
    try:
        ...
        rows = conn.execute(
            "SELECT folder_id, job_id, cmd_line FROM job_detail "
            "ORDER BY folder_id, job_id"
        ).fetchall()
        with conn:
            conn.execute("DELETE FROM job_detail_parsed")
            conn.execute("DELETE FROM parse_quality")
            for folder_id, job_id, cmd in rows:
                _parse_one(conn, folder_id, job_id, cmd, coverage)
    finally:
        conn.close()
    return coverage


def _parse_one(conn, folder_id, job_id, cmd, coverage: ParseCoverage) -> None:
    where = f"job {folder_id}.{job_id}"
    if not cmd or not str(cmd).strip():
        coverage.no_cmd_line += 1
        _quality(conn, folder_id, job_id, "no_cmd_line", 0, 0, 0, 0)
        return

    parsed = parse_command(str(cmd))
    unknown = 0
    ...
    if n_inv == 0 and n_fop == 0:
        verdict = "unparsed"
        coverage.unparsed += 1
        LOGGER.warning("%s: cmd_line yielded no invocation and no file op "
                       "(unparsed): %.120s", where, cmd)
    elif unknown or n_unparsed:
        verdict = "partial"
        coverage.partial += 1
        LOGGER.warning("%s: partial parse — %d UNKNOWN invocation(s), "
                       "%d unparsed statement(s)", where, unknown, n_unparsed)
    else:
        verdict = "parsed"
        coverage.parsed += 1
    _quality(conn, folder_id, job_id, verdict, n_inv, n_fop, unknown, n_unparsed)


def _quality(conn, folder_id, job_id, verdict, inv, fop, unknown, unparsed) -> None:
    conn.execute(
        "INSERT INTO parse_quality (folder_id, job_id, verdict, invocations, "
        "file_ops, unknown_invocations, unparsed_statements) "
        "VALUES (?, ?, ?, ?, ?, ?, ?)",
        (folder_id, job_id, verdict, inv, fop, unknown, unparsed),
    )
