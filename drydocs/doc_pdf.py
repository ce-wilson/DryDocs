...
Renders a design doc's ``.print.html`` (an L3 output) to ``.pdf`` with a headless
...
def html_to_pdf(html_path: str | Path, pdf_path: str | Path | None = None,
    ...
    """Render ``html_path`` (a .print.html) to a normalized PDF; return its path."""
    html_path = Path(html_path)
    if pdf_path is None:
        pdf_path = html_path.with_name(html_path.name.replace(".print.html", ".pdf"))
