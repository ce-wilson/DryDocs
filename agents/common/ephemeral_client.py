...
The *owner session* is the console user's PUBLIC ``session_id`` handle,
forwarded by the Ask spoke with the question (R5 wiring; ADR 0019). It names
the session the ref is scoped to and nothing more -- the agent authenticates
ITSELF with the agent key, so no console credential ever reaches this tier.
Without a handle there is no owner to scope the ref to — ``make_register``
returns None and the envelope's ``explore_ref`` fields stay null, which is
the honest pre-R5 state.
"""
...
def make_register(
    ...
    registration surface isn't configured (no owner session / no agent key).
    ...
    if not owner_session or not agent_key:
        return None
