...
def _row(**over) -> dict:
    base = {
        "job_name": "JOB_A", "job_id": "1", "path": "host_group",
        "group_name": "grp-x", "pinned_host": None, "folder": "F1",
        "node_id": "grp-x", "avg_start_time": None, "start_next_day": None,
        "avg_run_time": None, "window_start": None, "window_end": None,
    }
    base.update(over)
    return base
...
def test_report_places_job_stats_over_folder_window() -> None:
    rows = [_row(avg_start_time="01:00", avg_run_time=7200,
...
def test_unparseable_timing_is_its_own_finding() -> None:
    report = build_report(
        "group", "grp-x",
        {"group_jobs": [_row(avg_start_time="sometimes", window_start="junk",
                             window_end="junk")]},
    )
...
def test_hardcoded_pin_is_both_a_job_and_a_finding() -> None:
    pinned = _row(job_name="JOB_P", path="agent_host", pinned_host="host-1",
                  node_id="host-1", avg_start_time="04:00", avg_run_time=600)
...
def test_duplicate_rows_across_sections_dedupe() -> None:
    ...
    report = build_report("group", "grp-x",
                          {"group_jobs": [row, dict(row)]})
...
def test_committed_cypher_sections_exist() -> None:
    ...
    for name in (
        "host_exists", "group_exists", "group_dcs",
        "host_direct", "host_via_group", "group_jobs", "group_hardcoded",
        "xval_intent_without_edge", "xval_stale_edge_host", "xval_stale_edge_group",
    ):
