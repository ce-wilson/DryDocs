...
# ---- C7 (Phase 3 of the tech-debt review): structured fields at the gate ----
# Mappings confirmed AFTER this date (exclusive — the 2026-07-10 K3-gate batch
# predates the rule) must carry the structured fields the free-text forms used
# to hide: an entry-level vocab_id (or explicit ~ plus a reason FIELD — YAML
# comments are invisible to every guard here) and a capture field naming the
# config/taxonomy/ file, or waiving it explicitly with a reason (F5 policy;
# the platforms.yaml retrofit is the precedent for what silent erosion costs).
STRUCTURED_FIELDS_CUTOFF = "2026-07-10"


def _confirmed_after_cutoff(mappings: list) -> list:
    import datetime as dt

    cutoff = dt.date.fromisoformat(STRUCTURED_FIELDS_CUTOFF)
    post = []
    for m in mappings:
        confirmed = m.get("confirmed_on")
        if confirmed is None:
            continue
        if not isinstance(confirmed, dt.date):
            confirmed = dt.date.fromisoformat(str(confirmed))
        if confirmed > cutoff:
            post.append(m)
    return post


def test_post_cutoff_mappings_declare_vocab_id(map_doc: dict) -> None:
    """C7: every mapping confirmed after the cutoff must carry an entry-level
    `vocab_id` — a real id (resolution is test_vocab_id_resolves...'s job) or
    an explicit `~` accompanied by a non-empty `vocab_id_reason` note."""
    failures = []
    for m in _confirmed_after_cutoff(map_doc["mappings"]):
        if "vocab_id" not in m:
            ...
            if not (isinstance(reason, str) and reason.strip()):
                failures.append(
                    f"{m['id']}: vocab_id ~ without a vocab_id_reason field "
                    "(a YAML comment is not a reason note)"
                )
    assert not failures, "\n".join(failures)


def test_post_cutoff_mappings_declare_capture(map_doc: dict) -> None:
    """C7/F5: taxonomy-first means every newly confirmed mapping names its
    config/taxonomy/ capture file — or waives it EXPLICITLY with a reason
    (`capture: waived — <reason>`), never silently."""
    failures = []
    for m in _confirmed_after_cutoff(map_doc["mappings"]):
        ...
        if capture.startswith("waived"):
            if len(capture.split(None, 1)) < 2:
                failures.append(f"{m['id']}: capture waived without a reason")
        elif not capture.replace("\\", "/").startswith("config/taxonomy/"):
            failures.append(
                f"{m['id']}: capture '{capture}' is neither a config/taxonomy/ "
                "path nor a 'waived — <reason>' form"
            )
        elif not (REPO / capture).is_file():
            failures.append(f"{m['id']}: capture file '{capture}' does not exist")
    assert not failures, "\n".join(failures)
