...
def test_print_flags_are_fixed_and_headless() -> None:
    flags = print_flags("docs/design/x.print.html", "out.pdf")
    ...
    assert flags[-1].startswith("file://") and flags[-1].endswith("x.print.html")


def test_print_flags_deterministic() -> None:
    a = print_flags("a.print.html", "a.pdf")
    b = print_flags("a.print.html", "a.pdf")
    assert a == b
