...
def test_real_registry_gate_state() -> None:
    ...
    # airflow-mwaa: crosswalk gate signed off 2026-07-14 (source-row activation
    # only — loading still requires a loader + its own gate).
    for live in ("controlm-psgmgr", "seal-extract", "catalog-pat", "airflow-mwaa"):
        assert reg.is_confirmed(live), f"{live} should be confirmed"
    for placeholder in ("autosys-export", "oracle-schemas", "snowflake"):
