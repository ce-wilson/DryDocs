...
from drydocs_lineage.extract import LineageCandidate, extract_candidates
from drydocs_lineage.writer import write_curated


def test_package_surfaces() -> None:
    assert set(drydocs_lineage.__all__) == {"DATABASE", "curation", "extract", "writer"}
...
def test_both_components_share_the_core_parser() -> None:
    ...
    from drydocs_lineage import extract

    assert extract.parse_command is core_parser.parse_command
...
def test_stubs_raise_not_implemented() -> None:
    with pytest.raises(NotImplementedError):
        extract_candidates("JOB0001_SAMPLE", "run.sh a b")
    with pytest.raises(NotImplementedError):
        curate([], {})
    with pytest.raises(NotImplementedError):
        write_curated([])
...
def test_value_objects_carry_the_trust_contract() -> None:
    cand = LineageCandidate(
        job_name="JOB0001_SAMPLE", direction="reads",
        file_ref="/data/sample/in/file.tok", evidence="synthetic",
    )
    assert cand.status is CurationStatus.PROPOSED  # never born confirmed
