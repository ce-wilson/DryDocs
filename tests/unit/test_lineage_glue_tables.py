    ...
    return _write_csv(
        ...
        [
            [
                "104999",
                DB_RAW,
                f"{SEAL}_300000099_EVENTDATA_RAW_TRUSTED",
                GUID_RT,
                "datasetGuid",
...
def test_enriches_a_mac_staged_dataset_instead_of_duplicating(guid_sheet) -> None:
    ...
    g.add_data_asset(
        DataAssetNode(
            node_id=asset_id(MAC_DATASET_KIND, GUID_RT),
            kind=MAC_DATASET_KIND,
            location=GUID_RT,
            properties={"zone": "RAW", "dataset_name": "eventdata"},
        )
    )
    cov = GlueTableInventoryExtractor().extract(guid_sheet, g)
    node = g.data_assets[asset_id(MAC_DATASET_KIND, GUID_RT)]
...
def consumption_sheet(tmp_path: Path) -> Path:
    ...
    return _write_csv(
        ...
        ["database", "databasePlatformId", "databaseName", "tableName", "databaseFullPath"],
        [
            ["AWS-S3", "104999", DB_REFINED, "ACCT_SYNTH", f"{DB_REFINED}.ACCT_SYNTH"],
...
def test_path_rows_stage_glue_tables_and_join_guid_placements(
    ...
    standalone = g.data_assets[asset_id(GLUE_TABLE_KIND, f"{DB_REFINED.lower()}.acct_synth")]
...
def load_sheet(tmp_path: Path) -> Path:
    ...
    return _write_csv(
        ...
        [
            ...
            "tableDatasetId",
            "tableDatasetIdSource",
            "fullPath *formula except for AWS",
        ],
        [
            [
                "AWS-S3",
...
def test_load_sheet_headers_seal_facts_and_idsource_routing(load_sheet) -> None:
    ...
    assert asset_id(GLUE_TABLE_KIND, f"{DB_RAW.lower()}.other_keyed_table") in g.data_assets
    node = g.data_assets[asset_id(MAC_DATASET_KIND, GUID_RT)]
